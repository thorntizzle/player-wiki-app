...
- the Systems UI now also exposes a player-visible XGE `Book Chapters` slice that combines wrapper-first atomic pages with the broader `Tool Proficiencies`, `Spellcasting`, `Encounter Building`, and `Downtime Revisited` chapter pages, while `Random Encounters: A World of Possibilities`, `Traps Revisited`, `Awarding Magic Items`, and the readable `Shared Campaigns` wrapper stay as follow-up work
