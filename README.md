...
- the Systems UI now also exposes player-visible XGE and TCE `Book Chapters` slices through the normal source/category/detail flow; XGE includes the readable `Shared Campaigns` wrapper plus `Tool Proficiencies`, `Spellcasting`, `Encounter Building`, `Random Encounters: A World of Possibilities`, `Traps Revisited`, `Downtime Revisited`, and `Awarding Magic Items`, while the current TCE slice starts with `Ten Rules to Remember`
