...
- the Systems UI now also exposes a player-visible XGE `Book Chapters` slice that combines wrapper-first atomic pages with the broader `Tool Proficiencies`, `Spellcasting`, `Encounter Building`, `Random Encounters: A World of Possibilities`, `Traps Revisited`, `Downtime Revisited`, and `Awarding Magic Items` chapter pages, while the readable `Shared Campaigns` wrapper stays as follow-up work
