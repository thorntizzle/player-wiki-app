...
import { apiTokenRoleForCampaign, createUser, deleteCharacterAssignment, deleteUser, disableUser, enableUser, getActiveUserById, getCharacterAssignment, getMembership, getUserByEmail, getUserById, hasActivePlayerMembership, insertAuthAuditLog, issueInviteToken, issuePasswordResetToken, listActiveMembershipsForUser, listActivePlayerMembershipUsers, readApiTokenAuthContext, revokeAllUserApiTokens, revokeAllUserSessions, upsertCharacterAssignment, upsertMembership, updateApiTokenAccountSettings, } from "./auth/repository.js";
import { buildApiTokenAccountSettingsPayload, buildApiTokenMePayload, buildApiTokenViewAsState, buildFixtureAccountSettingsPayload, buildFixtureMePayload, } from "./auth/view.js";
import { getApiConfig } from "./config.js";
import { buildCampaignControlPayload, campaignRoleCanAccessScope, campaignRoleCanManageVisibility, campaignScopeIsPublic, updateCampaignVisibilitySettings, } from "./campaigns/control.js";
import { getCampaignBySlug, listCampaigns, listCampaignSlugs } from "./campaigns/repository.js";
import { addCombatCondition, addNpcCombatant, addPlayerCombatant, addStatblockCombatant, addSystemsMonsterCombatant, advanceCombatTurn, buildCombatReadOnlyPayload, clearCombatTracker, deleteCombatCondition, deleteCombatant, setCurrentCombatant, supportsCombatTracker, updateCombatCondition, updateCombatantNpcResources, updateCombatantPlayerDetailVisibility, updateCombatantResources, updateCombatantTurn, updateCombatantVitals, } from "./combat/view.js";
import { buildDmContentPayload, createDmContentCondition, createDmContentStatblock, deleteDmContentCondition, deleteDmContentStatblock, updateDmContentCondition, updateDmContentStatblock, } from "./dmContent/view.js";
...
import { buildSessionLogDetailPayload, buildSessionStatePayload, clearRevealedSessionArticles, closeSession, createSessionArticle, deleteSessionArticle, deleteSessionLog, postSessionMessage, readSessionArticleImage, revealSessionArticle, startSession, updateSessionArticle, } from "./session/view.js";
...
import { archiveCustomSystemsEntry, buildDmContentSystemsPayload, createCustomSystemsEntry, importCampaignItemMechanics, restoreCustomSystemsEntry, updateCustomSystemsEntry, } from "./systems/management.js";
import { buildCombatSystemsMonsterSearchPayload, buildCampaignSystemsEntryDetailPayload, buildCampaignSystemsIndexPayload, buildCampaignSystemsSourceCategoryPayload, buildCampaignSystemsSourceDetailPayload, buildCampaignSystemsSourceListPayload, updateCampaignSystemsEntryOverride, updateCampaignSystemsSources, } from "./systems/sources.js";
import { advancedEditorUnsupportedMessage, applyCharacterAdvancedEditorReferenceUpdate, applyCharacterCultivationAction, buildCharacterAdvancedEditorPayload, buildCharacterAdvancementShellPayload, buildCharacterAuthoringLinks, buildCharacterCultivationShellPayload, buildDndCharacterCreateContext, buildDndCreateCharacter, buildXianxiaCreateCharacter, listXianxiaCultivationGenericTechniqueRows, listXianxiaCultivationMartialArtRows, listXianxiaCreateGenericTechniqueOptions, buildXianxiaManualImportCharacter, buildXianxiaManualImportContext, characterAdvancedEditorIsSupported, characterCultivationIsSupported, listXianxiaManualImportMartialArtOptions, nativeCharacterCreateLane, nativeCharacterCreateUnsupportedMessage, xianxiaKnownGenericTechniqueOptionKeys, } from "./content/characterAuthoring.js";
import { getCampaignConfigFile } from "./content/repository.js";
import { buildCampaignContentPageRemovalSafety, createCampaignContentCharacter, deleteCampaignContentAsset, deleteCampaignContentCharacter, deleteCampaignContentCharacterPortrait, deleteCampaignContentPage, getCampaignContentAsset, getCampaignContentCharacter, getCampaignContentPage, listCampaignContentAssets, listCampaignContentCharacters, listCampaignContentPages, readCampaignProtectedAsset, sanitizeContentAssetRef, sanitizeContentCharacterSlug, sanitizeContentPageRef, updateCampaignConfigFile, validateCampaignContentCharacterPortraitUpload, writeCampaignCharacterDefinitionFile, writeCampaignContentAsset, writeCampaignContentCharacter, writeCampaignContentCharacterPortrait, writeCampaignContentPage, } from "./content/repository.js";
import { applyCharacterSessionRest, canEditCharacterSessionState, previewCharacterRest, readCharacterStateSnapshot, updateCharacterAdvancedEditorReferenceState, updateCharacterCultivationDefinitionState, updateCharacterSheetEdit, updateCharacterPortraitRevision, updateCharacterSessionArtificerInfusions, updateCharacterSessionCurrency, updateCharacterSessionEquipment, updateCharacterSessionFeatureState, updateCharacterSessionInventory, updateCharacterSessionNotes, updateCharacterSessionPersonal, updateCharacterSessionResource, updateCharacterSessionSpellSlots, updateCharacterSessionXianxiaActiveState, updateCharacterSessionXianxiaDaoImmolatingUseRecord, updateCharacterSessionXianxiaDaoImmolatingUseRequest, updateCharacterSessionXianxiaInventoryAdd, updateCharacterSessionXianxiaInventoryEquipped, updateCharacterSessionXianxiaInventoryItem, updateCharacterSessionXianxiaInventoryRemove, updateCharacterSessionVitals, } from "./content/characterState.js";
import { buildCampaignConfigPayload, buildCharacterDetailPayload, buildCharacterRosterPayload, buildContentAssetDeletePayload, buildContentAssetDetailPayload, buildContentAssetListPayload, buildContentAssetWritePayload, buildContentCharacterDeletePayload, buildContentCharacterDetailPayload, buildContentCharacterListPayload, buildContentPageDeletePayload, buildContentPageDetailPayload, buildContentPageListPayload, } from "./content/view.js";
import { campaignWikiRepository, sectionSortKey, setWikiConfig, slugify } from "./wiki/repository.js";
import { serializeCampaign, serializePublicWikiPage, serializePublicWikiSectionGroup, serializeSectionNavigation, splitPagesBySubsection, } from "./wiki/view.js";
const app = new Hono();
...
function utcIsoTimestamp(value = new Date()) {
    const normalized = new Date(value);
    normalized.setMilliseconds(0);
    return normalized.toISOString().replace(/\.\d{3}Z$/, "+00:00");
}
function buildManagedCharacterImportMetadata(campaignSlug, characterSlug, currentImportMetadata) {
    return {
        campaign_slug: campaignSlug,
        character_slug: characterSlug,
        source_path: typeof currentImportMetadata.source_path === "string" && currentImportMetadata.source_path.trim().length > 0
...
function buildPreservedCharacterImportMetadata(campaignSlug, characterSlug, currentImportMetadata) {
    return {
        ...
        source_path: typeof currentImportMetadata.source_path === "string" && currentImportMetadata.source_path.trim().length > 0
            ? currentImportMetadata.source_path.trim()
            : `managed://${campaignSlug}/${characterSlug}`,
        imported_at_utc: typeof currentImportMetadata.imported_at_utc === "string" && currentImportMetadata.imported_at_utc.trim().length > 0
            ? currentImportMetadata.imported_at_utc.trim()
            : utcIsoTimestamp(),
        parser_version: typeof currentImportMetadata.parser_version === "string" && currentImportMetadata.parser_version.trim().length > 0
            ? currentImportMetadata.parser_version.trim()
            : "api-v1",
        import_status: typeof currentImportMetadata.import_status === "string" && currentImportMetadata.import_status.trim().length > 0
...
function campaignNotFound(campaignSlug) {
    return jsonError("campaign_not_found", `Could not load campaign '${campaignSlug}' from fixture directory.`, 404, {
        campaign_slug: campaignSlug,
        campaigns_dir: config.campaignsDir,
    });
}
...
async function readJsonObject(ctx) {
    try {
...
async function readOptionalJsonObject(ctx) {
...
function applyViewAsCookieState(ctx, state) {
    if (state.shouldClearCookie) {
        clearRequestedViewAsUserId(ctx);
    }
}
function viewAsRoleForCampaign(memberships, campaignSlug) {
...
function apiTokenCampaignIdentity(ctx, authContext, campaignSlug) {
...
const viewAsReadOnlyGuard = async (ctx, next) => {
...
app.use("/api/v1/campaigns/*", viewAsReadOnlyGuard);
function roleResolutionError(result) {
    if (result.kind === "forbidden") {
        return forbidden(result.message);
    }
    return authRequired();
}
function writeResolutionError(result) {
    if (result.kind === "forbidden") {
        return forbidden(result.message);
    }
    return authRequired();
}
function toFixtureSystemsRole(role) {
    return role;
}
function resolveSessionManagerBearerWrite(ctx, campaignSlug, fixtureForbiddenMessage) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
...
function resolveCampaignRole(ctx, campaignSlug) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
...
function resolveContentManagerRole(ctx, campaignSlug) {
...
function resolveContentManagerBearerWrite(ctx, campaignSlug, fixtureForbiddenMessage) {
    const forbiddenMessage = "You do not have permission to manage campaign content.";
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
        const role = apiTokenRoleForCampaign(apiAuth.context, campaignSlug);
...
function resolveCharacterControlsAssignmentWrite(ctx) {
...
function resolveCharacterControlsDeleteWrite(ctx, campaignSlug) {
...
function resolveDmContentBearerWrite(ctx, campaignSlug) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
        const role = apiTokenRoleForCampaign(apiAuth.context, campaignSlug);
        if (role === "admin" || role === "dm") {
...
function resolveSystemsManagerBearerWrite(ctx, campaignSlug) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
        const role = apiTokenRoleForCampaign(apiAuth.context, campaignSlug);
        if (role === "admin" || role === "dm") {
...
function resolveCombatManagerBearerWrite(ctx, campaignSlug) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
        const role = apiTokenRoleForCampaign(apiAuth.context, campaignSlug);
        if (role === "admin" || role === "dm") {
...
function resolveCombatBearerWrite(ctx, campaignSlug) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
        const role = apiTokenRoleForCampaign(apiAuth.context, campaignSlug);
        if (role) {
...
function resolveCharacterSessionBearerWrite(ctx, campaignSlug) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
        const role = apiTokenRoleForCampaign(apiAuth.context, campaignSlug);
        if (role) {
...
function resolveCampaignVisibilityManagerRole(ctx, campaign) {
...
function resolveCampaignVisibilityWriter(ctx, campaign) {
    const forbiddenMessage = "You do not have permission to manage campaign visibility.";
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
...
function resolveAppAdminAuth(ctx) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
        if (apiAuth.context.user.is_admin) {
            return {
...
function resolveAppAdminBearerWrite(ctx) {
    const apiAuth = readApiTokenAuthContext(config.dbPath, ctx.req.header("Authorization"));
    if (apiAuth.kind === "authenticated") {
        if (apiAuth.context.user.is_admin) {
            return {
...
function supportsCharacterControlsRoutes(_system) {
    return true;
}
function requestQueryValues(ctx) {
...
const XIANXIA_CREATE_ATTRIBUTE_KEYS = ["str", "dex", "con", "int", "wis", "cha"];
const XIANXIA_CREATE_ATTRIBUTE_LABELS = {
    str: "Strength",
    dex: "Dexterity",
    con: "Constitution",
    int: "Intelligence",
...
const XIANXIA_CREATE_EFFORT_KEYS = ["basic", "weapon", "guns_explosive", "magic", "ultimate"];
const XIANXIA_CREATE_EFFORT_LABELS = {
    basic: "Basic",
    weapon: "Weapon",
...
const XIANXIA_CREATE_ENERGY_KEYS = ["jing", "qi", "shen"];
const XIANXIA_CREATE_ENERGY_LABELS = {
    jing: "Jing",
    qi: "Qi",
    shen: "Shen",
};
...
function buildCharacterCreateTools(campaign, canAuthorCharacters) {
...
function buildXianxiaCharacterCreateContext({ dbPath, campaign, campaignConfig, values, }) {
    ...
    return {
        ...
        generic_technique_options: listXianxiaCreateGenericTechniqueOptions(dbPath, campaign, campaignConfig, [normalizedValues.gm_granted_generic_technique_entry_keys].filter(Boolean)),
...
function buildCharacterControlsPayload(campaignSlug, record, message) {
...
function buildCharacterReadControlsPayload(campaign, record, auth) {
    const campaignSlug = campaign.slug;
    const characterSlug = record.character_slug;
    const assignment = getCharacterAssignment(config.dbPath, campaignSlug, characterSlug);
    const assignedUser = assignment ? getUserById(config.dbPath, assignment.user_id) : null;
    const canAssignOwner = auth.role === "admin";
    const canDeleteCharacter = auth.role === "admin" ||
...
function canReadCharacterDetail(campaign, characterSlug, auth) {
    if (campaignRoleCanAccessScope(config.dbPath, campaign, auth.role, "characters")) {
        return true;
    }
    const hasSessionModeSurfaceAccess = campaignRoleCanAccessScope(config.dbPath, campaign, auth.role, "session") ||
...
async function serializePageList(campaign, pages) {
    return Promise.all(pages.map((page) => serializePublicWikiPage(campaign, page, {
        includeImage: false,
        assetsExist: (assetRef) => assetExists(campaign, assetRef),
    })));
}
function pageSlugFromWildcard(pathname, campaignSlug) {
...
function contentPageRefFromWildcard(pathname, campaignSlug) {
    const prefix = `/api/v1/campaigns/${campaignSlug}/content/pages/`;
    if (!pathname.startsWith(prefix)) {
        return "";
    }
    try {
        return (sanitizeContentPageRef(pathname.slice(prefix.length)) || "");
    }
    catch {
        return "";
    }
}
function rawContentPageRefFromWildcard(pathname, campaignSlug) {
    const prefix = `/api/v1/campaigns/${campaignSlug}/content/pages/`;
    if (!pathname.startsWith(prefix)) {
        return "";
    }
...
function contentAssetRefFromWildcard(pathname, campaignSlug) {
    const prefix = `/api/v1/campaigns/${campaignSlug}/content/assets/`;
    if (!pathname.startsWith(prefix)) {
        return "";
    }
    try {
        return (sanitizeContentAssetRef(pathname.slice(prefix.length)) || "");
    }
    catch {
        return "";
    }
}
function campaignAssetPathFromWildcard(pathname, campaignSlug) {
...
function rawContentAssetRefFromWildcard(pathname, campaignSlug) {
    const prefix = `/api/v1/campaigns/${campaignSlug}/content/assets/`;
    if (!pathname.startsWith(prefix)) {
        return "";
    }
...
function systemsEntryOverrideKeyFromWildcard(pathname, campaignSlug) {
...
function contentPageNotFound(campaignSlug, pageRef) {
    return jsonError("content_page_not_found", `Could not find content page '${pageRef}' in campaign '${campaignSlug}'.`, 404, { campaign_slug: campaignSlug, page_ref: pageRef });
}
function hardDeleteBlocked(pageRef, removalSafety) {
    return jsonError("hard_delete_blocked", "Hard delete blocked for this content page.", 409, {
        page_ref: pageRef,
...
function contentAssetNotFound(campaignSlug, assetRef) {
    return jsonError("content_asset_not_found", `Could not find content asset '${assetRef}' in campaign '${campaignSlug}'.`, 404, { campaign_slug: campaignSlug, asset_ref: assetRef });
}
function contentCharacterNotFound(campaignSlug, characterSlug) {
    return jsonError("content_character_not_found", `Could not find content character '${characterSlug}' in campaign '${campaignSlug}'.`, 404, { campaign_slug: campaignSlug, character_slug: characterSlug });
}
function parseLiveRevisionHeader(ctx) {
...
function parseLiveViewTokenHeader(ctx) {
...
app.get(ROUTES.appState, async (ctx) => ctx.json({
    ok: true,
    app: {
        ...config.app,
...
app.post(ROUTES.adminUserMembership, async (ctx) => {
    ...
    const eventType = previous === null || previous.status === "removed"
        ? "membership_created"
...
app.post(ROUTES.adminUserEnable, async (ctx) => {
    ...
    return ctx.json({
        ...payload,
        message: enabledUser.status === "active"
...
async function systemsIndexResponse(ctx) {
    ...
    return ctx.json({
        ok: true,
        ...buildCampaignSystemsIndexPayload(config.dbPath, campaign, campaignConfig?.config || {}, role, ctx.req.query("q") || "", ctx.req.query("reference_q") || ""),
    });
}
...
app.put(ROUTES.systemsSources, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = updateCampaignSystemsSources(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, auth.actorUserId || 0, jsonPayload.payload);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.put(ROUTES.systemsEntryOverrideUpdate, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = updateCampaignSystemsEntryOverride(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, auth.actorUserId || 0, systemsEntryOverrideKeyFromWildcard(ctx.req.path, campaign.slug), jsonPayload.payload);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.systemsCustomEntries, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = createCustomSystemsEntry(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, auth.actorUserId || 0, jsonPayload.payload);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.systemsItemMechanicsImport, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = importCampaignItemMechanics(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, auth.actorUserId || 0, jsonPayload.payload);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.put(ROUTES.systemsCustomEntry, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = updateCustomSystemsEntry(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, auth.actorUserId || 0, ctx.req.param("entrySlug") || "", jsonPayload.payload);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.systemsCustomEntryArchive, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = archiveCustomSystemsEntry(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, auth.actorUserId || 0, ctx.req.param("entrySlug") || "");
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.systemsCustomEntryRestore, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = restoreCustomSystemsEntry(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, auth.actorUserId || 0, ctx.req.param("entrySlug") || "");
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.get(ROUTES.systemsSourceDetail, async (ctx) => {
    ...
    const role = auth.role;
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = buildCampaignSystemsSourceDetailPayload(config.dbPath, campaign, campaignConfig?.config || {}, ctx.req.param("sourceId") || "", role, ctx.req.query("reference_q") || "");
...
app.get(ROUTES.systemsSourceCategory, async (ctx) => {
    ...
    const role = auth.role;
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = buildCampaignSystemsSourceCategoryPayload(config.dbPath, campaign, campaignConfig?.config || {}, ctx.req.param("sourceId") || "", ctx.req.param("entryType") || "", role, ctx.req.query("q") || "");
    if (result.status === "forbidden") {
        const error = forbidden(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.get(ROUTES.systemsEntryDetail, async (ctx) => {
    ...
    const role = auth.role;
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = buildCampaignSystemsEntryDetailPayload(config.dbPath, campaign, campaignConfig?.config || {}, ctx.req.param("entrySlug") || "", role);
    if (result.status === "forbidden") {
        const error = forbidden(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.get(ROUTES.combatSystemsMonsterSearch, async (ctx) => {
    ...
    const role = auth.role;
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = buildCombatSystemsMonsterSearchPayload(config.dbPath, campaign, campaignConfig?.config || {}, role, ctx.req.query("q") || "");
    if (result.status === "forbidden") {
        const error = forbidden(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
function combatUnchangedResponse(payload) {
    return {
        ok: true,
        changed: false,
...
app.get(ROUTES.combatState, async (ctx) => {
    ...
    if (requestedRevision === payload.live_revision &&
...
app.post(ROUTES.combatAdvanceTurn, async (ctx) => {
    ...
    if (!supportsCombatTracker(campaign.system)) {
        const error = validationError(`Combat tracker support for ${campaign.system || "this system"} is not available yet.`);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.combatSystemsMonsters, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = addSystemsMonsterCombatant(config.dbPath, campaign, campaignConfig?.config || {}, jsonPayload.payload, actorUserId);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.combatCombatantVitals, async (ctx) => {
    ...
    const result = await updateCombatantVitals(config, campaign.slug, combatantId, jsonPayload.payload, actorUserId, auth.role);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.combatCombatantResources, async (ctx) => {
    ...
    const result = updateCombatantResources(config.dbPath, campaign.slug, combatantId, jsonPayload.payload, actorUserId, auth.role);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.combatCombatantNpcResources, async (ctx) => {
    ...
    const result = updateCombatantNpcResources(config.dbPath, campaign.slug, combatantId, jsonPayload.payload, actorUserId, auth.role);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.combatCombatantPlayerDetailVisibility, async (ctx) => {
    ...
    const result = updateCombatantPlayerDetailVisibility(config.dbPath, campaign.slug, combatantId, jsonPayload.payload, actorUserId, auth.role);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.combatCombatantConditions, async (ctx) => {
    ...
    const result = addCombatCondition(config.dbPath, campaign.slug, combatantId, jsonPayload.payload, actorUserId, auth.role);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.combatConditionUpdate, async (ctx) => {
    ...
    const result = updateCombatCondition(config.dbPath, campaign.slug, conditionId, jsonPayload.payload, actorUserId, auth.role);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.campaignControlVisibility, async (ctx) => {
    ...
    const result = updateCampaignVisibilitySettings(config.dbPath, campaign, auth.role, auth.actorUserId, rawVisibility);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.get(ROUTES.wikiHome, async (ctx) => {
    ...
    const groupedSections = await Promise.all(sortSectionNames(grouped.keys()).map((sectionName) => serializePublicWikiSectionGroup(campaign, sectionName, grouped.get(sectionName) || [], {
        assetsExist: (assetRef) => assetExists(campaign, assetRef),
    })));
...
app.get(ROUTES.wikiSection, async (ctx) => {
    ...
    if (!pages.length) {
        const error = jsonError("wiki_section_not_found", `Could not find wiki section '${sectionSlug}' in campaign '${campaignSlug}'.`, 404, { campaign_slug: campaignSlug, section_slug: sectionSlug });
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.get(ROUTES.wikiPage, async (ctx) => {
    ...
    if (!page || bodyHtml === null) {
        const error = jsonError("wiki_page_not_found", `Could not find wiki page '${pageSlug}' in campaign '${campaignSlug}'.`, 404, { campaign_slug: campaignSlug, page_slug: pageSlug });
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
    const navigationPages = await campaignWikiRepository.searchPages(campaignSlug, "");
...
app.get(ROUTES.sessionState, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const payload = await buildSessionStatePayload(config.dbPath, campaign, campaignConfig?.config || {}, role, auth.kind === "authenticated" ? auth.actorUserId || null : null);
    const requestedRevision = parseLiveRevisionHeader(ctx);
    const requestedViewToken = parseLiveViewTokenHeader(ctx);
    if (requestedRevision === payload.session_revision &&
        requestedViewToken.length > 0 &&
...
app.put(ROUTES.dmContentConditionUpdate, async (ctx) => {
    ...
    const result = updateDmContentCondition(config.dbPath, campaign.slug, conditionDefinitionId, jsonPayload.payload, auth.actorUserId || 0);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.sessionMessageCreate, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = await postSessionMessage(config.dbPath, campaign, campaignConfig?.config || {}, toFixtureSystemsRole(role), {
        id: apiAuth.context.user.id,
        display_name: apiAuth.context.user.display_name,
    }, jsonPayload.payload);
    if (result.status === "validation_error") {
...
app.post(ROUTES.sessionArticleCreate, async (ctx) => {
    ...
    const auth = resolveSessionManagerBearerWrite(ctx, campaign.slug, "Session article writes require bearer API authentication.");
    if (auth.kind === "error") {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = await createSessionArticle(config.dbPath, config.campaignsDir, campaign, campaignConfig?.config || {}, auth.role, { id: auth.actor.id }, jsonPayload.payload);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.put(ROUTES.sessionArticleUpdate, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = await updateSessionArticle(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, { id: auth.actor.id }, articleId, jsonPayload.payload);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.sessionArticleReveal, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = await revealSessionArticle(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, auth.actor, articleId);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.delete(ROUTES.sessionArticlesRevealedClear, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = await clearRevealedSessionArticles(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, { id: auth.actor.id });
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.delete(ROUTES.sessionArticleDelete, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = await deleteSessionArticle(config.dbPath, campaign, campaignConfig?.config || {}, auth.role, { id: auth.actor.id }, articleId);
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.get(ROUTES.sessionLogDetail, async (ctx) => {
    ...
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = await buildSessionLogDetailPayload(config.dbPath, campaign, campaignConfig?.config || {}, sessionId, role);
    if (result.status === "not_found") {
        const error = notFound("session_log_not_found", "Could not find that session log.");
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.get(ROUTES.sessionArticleSourceSearch, async (ctx) => {
    ...
    const role = auth.role;
    const campaignConfig = await getCampaignConfigFile(config, campaign.slug);
    const result = await buildSessionArticleSourceSearchPayload(config.dbPath, campaign, campaignConfig?.config || {}, role, ctx.req.query("q") || "");
    if (result.status === "forbidden") {
...
app.patch(ROUTES.campaignConfigUpdate, async (ctx) => {
    ...
    const auth = resolveContentManagerBearerWrite(ctx, campaignConfig.campaign_slug, "Content config writes require bearer API authentication.");
    if (auth.kind !== "authenticated") {
        const error = roleResolutionError(auth);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
    ...
    const updates = typeof jsonPayload.payload.config === "object" &&
...
app.put(ROUTES.contentAssetUpdate, async (ctx) => {
    ...
    const auth = resolveContentManagerBearerWrite(ctx, campaign.slug, "Content asset writes require bearer API authentication.");
    if (auth.kind !== "authenticated") {
        const error = roleResolutionError(auth);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.put(ROUTES.contentPageUpdate, async (ctx) => {
    ...
    const auth = resolveContentManagerBearerWrite(ctx, campaign.slug, "Content page writes require bearer API authentication.");
    if (auth.kind !== "authenticated") {
        const error = roleResolutionError(auth);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.delete(ROUTES.contentPageDelete, async (ctx) => {
    ...
    const force = parseBooleanFlag(new URL(ctx.req.url).searchParams.get("force")) ||
...
app.get(ROUTES.characterRoster, async (ctx) => {
    ...
    const stateBySlug = new Map(rosterCharacters.map((record) => [
    ...
    return ctx.json(buildCharacterRosterPayload({
        campaign,
        records: rosterCharacters,
        stateBySlug,
        assetByRef,
        query,
        canManageSession: (auth.role === "admin" || auth.role === "dm") &&
...
app.get(ROUTES.characterCreateContext, async (ctx) => {
    ...
    const canAuthorCharacters = campaignRoleCanAccessScope(config.dbPath, campaign, auth.role, "characters") &&
        (auth.role === "admin" ||
...
app.post(ROUTES.characterCreate, async (ctx) => {
    ...
    const rawValues = payload.values !== null &&
    ...
    const createResult = await createCampaignContentCharacter(config, campaign.slug, String(createPayload.definition.character_slug || ""), createPayload.definition, createPayload.importMetadata, createPayload.initialState);
...
app.get(ROUTES.characterXianxiaManualImportContext, async (ctx) => {
    ...
    if (lane !== "xianxia") {
        const error = jsonError("unsupported_campaign_system", "Manual Xianxia character import is only available for Xianxia campaigns.", 400);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
    const configRecord = await getCampaignConfigFile(config, campaign.slug);
    return ctx.json({
...
app.post(ROUTES.characterXianxiaManualImport, async (ctx) => {
    ...
    const rawValues = nestedValues !== null &&
    ...
    const values = Object.fromEntries(Object.entries(rawValues).map(([key, value]) => [String(key), value === null || value === undefined ? "" : String(value)]));
    ...
    if (confirmImport) {
        const createResult = await createCampaignContentCharacter(config, campaign.slug, String(importResult.definition.character_slug || ""), importResult.definition, importResult.importMetadata, importResult.initialState);
        if (createResult.status === "not_found") {
            const error = campaignNotFound(campaignSlug);
            return ctx.json({ ok: error.ok, error: error.error }, error.status);
        }
...
app.get(ROUTES.characterDetail, async (ctx) => {
    ...
    const assets = (await listCampaignContentAssets(config, campaign.slug)) ?? [];
    const assetByRef = new Map(assets.map((asset) => [asset.asset_ref, asset]));
    const canManageSession = (auth.role === "admin" || auth.role === "dm") &&
        campaignRoleCanAccessScope(config.dbPath, campaign, auth.role, "session");
    ...
    return ctx.json(buildCharacterDetailPayload({
        campaign,
        record: character,
        stateRecord,
        assetByRef,
...
app.put(ROUTES.characterAdvancedEditorUpdate, async (ctx) => {
    ...
    const campaignPageRecords = await listCampaignContentPages(config, campaign.slug) ?? [];
    const referenceUpdate = applyCharacterAdvancedEditorReferenceUpdate(character.definition, jsonPayload.payload, campaignPageRecords);
    if (referenceUpdate.status === "validation_error") {
        const error = validationError(referenceUpdate.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
    const stateResult = updateCharacterAdvancedEditorReferenceState(config, campaign.slug, characterSlug, referenceUpdate.definition, {
...
async function handleCharacterAdvancementShellSubmit(ctx, kind, forbiddenMessage, fallbackMessage) {
    const campaignSlug = ctx.req.param("campaignSlug") || "";
    const campaign = await getCampaignBySlug(config, campaignSlug);
    if (!campaign) {
        const error = campaignNotFound(campaignSlug);
...
app.post(ROUTES.characterRetrainingSubmit, async (ctx) => handleCharacterAdvancementShellSubmit(ctx, "retraining", "You do not have permission to retrain this character.", "This character is not ready for Gen2 retraining."));
app.post(ROUTES.characterLevelUpSubmit, async (ctx) => handleCharacterAdvancementShellSubmit(ctx, "level_up", "You do not have permission to level up this character.", "This character is not ready for Gen2 level-up."));
app.post(ROUTES.characterProgressionRepairSubmit, async (ctx) => handleCharacterAdvancementShellSubmit(ctx, "progression_repair", "You do not have permission to repair progression for this character.", "This character is not ready for Gen2 progression repair."));
app.get(ROUTES.characterCultivation, async (ctx) => {
    ...
    const cultivationPayload = buildCharacterCultivationShellPayload({
        ...
        definition: character.definition,
        state: stateRecord.state,
        genericTechniqueOptions: listXianxiaCreateGenericTechniqueOptions(config.dbPath, campaign, configRecord?.config || {}, xianxiaKnownGenericTechniqueOptionKeys(character.definition)),
    });
    return ctx.json({
        ok: true,
        campaign,
...
app.post(ROUTES.characterCultivation, async (ctx) => {
    ...
    const stateResult = updateCharacterCultivationDefinitionState(config, campaign.slug, characterSlug, character.definition, cultivationAction.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (stateResult.status === "state_conflict") {
        const error = stateConflict(stateResult.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
    ...
    const writeResult = await writeCampaignCharacterDefinitionFile(config, campaign.slug, characterSlug, cultivationAction.definition);
    if (writeResult.status === "not_found") {
        const error = contentCharacterNotFound(campaign.slug, characterSlug);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
    ...
    const cultivationPayload = buildCharacterCultivationShellPayload({
        ...
        definition: writeResult.record.definition,
        state: stateResult.state,
        genericTechniqueOptions: listXianxiaCreateGenericTechniqueOptions(config.dbPath, campaign, campaignConfig, xianxiaKnownGenericTechniqueOptionKeys(writeResult.record.definition)),
    });
    return ctx.json({
        ok: true,
        campaign,
...
app.put(ROUTES.contentCharacterUpdate, async (ctx) => {
    ...
    const auth = resolveContentManagerBearerWrite(ctx, campaign.slug, "Content character writes require bearer API authentication.");
    if (auth.kind !== "authenticated") {
        const error = roleResolutionError(auth);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
    ...
    const result = await writeCampaignContentCharacter(config, campaign.slug, ctx.req.param("characterSlug") || "", jsonPayload.payload);
    if (result.status === "not_found") {
        const error = campaignNotFound(campaignSlug);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.delete(ROUTES.contentCharacterDelete, async (ctx) => {
    ...
    const result = await deleteCampaignContentCharacter(config, campaign.slug, ctx.req.param("characterSlug") || "");
    if (result.status === "not_found") {
        const characterSlug = sanitizeContentCharacterSlug(ctx.req.param("characterSlug") || "") || "";
        const error = contentCharacterNotFound(campaign.slug, characterSlug);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
...
app.post(ROUTES.characterControlsAssignment, async (ctx) => {
    ...
    return ctx.json(buildCharacterControlsPayload(campaign.slug, character, `Assigned ${characterSlug} to ${targetUser.email}.`));
});
...
app.get(ROUTES.characterRestPreview, async (ctx) => {
    ...
    const result = previewCharacterRest(config, campaign.slug, characterSlug, character.definition, ctx.req.param("restType") || "");
    if (result.status === "validation_error") {
        const error = validationError(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.characterSessionRest, async (ctx) => {
    ...
    const result = applyCharacterSessionRest(config, campaign.slug, characterSlug, character.definition, ctx.req.param("restType") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSheetEdit, async (ctx) => {
    ...
    const result = updateCharacterSheetEdit(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionVitals, async (ctx) => {
    ...
    const result = updateCharacterSessionVitals(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionResource, async (ctx) => {
    ...
    const result = updateCharacterSessionResource(config, campaign.slug, characterSlug, character.definition, ctx.req.param("resourceId") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionSpellSlots, async (ctx) => {
    ...
    const result = updateCharacterSessionSpellSlots(config, campaign.slug, characterSlug, character.definition, ctx.req.param("level") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionInventory, async (ctx) => {
    ...
    const result = updateCharacterSessionInventory(config, campaign.slug, characterSlug, character.definition, ctx.req.param("itemId") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionXianxiaActiveState, async (ctx) => {
    ...
    const result = updateCharacterSessionXianxiaActiveState(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.characterSessionXianxiaDaoImmolatingUseRequests, async (ctx) => {
    ...
    const result = updateCharacterSessionXianxiaDaoImmolatingUseRequest(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.characterSessionXianxiaDaoImmolatingUseRecords, async (ctx) => {
    ...
    const auth = resolveSessionManagerBearerWrite(ctx, campaign.slug, "Character session state writes require bearer API authentication.");
    if (auth.kind === "error") {
        return ctx.json({ ok: auth.error.ok, error: auth.error.error }, auth.error.status);
    }
    const characterSlug = sanitizeContentCharacterSlug(ctx.req.param("characterSlug") || "") || "";
    ...
    const result = updateCharacterSessionXianxiaDaoImmolatingUseRecord(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actor.id);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionCurrency, async (ctx) => {
    ...
    const result = updateCharacterSessionCurrency(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionNotes, async (ctx) => {
    ...
    const result = updateCharacterSessionNotes(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionPersonal, async (ctx) => {
    ...
    const result = updateCharacterSessionPersonal(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionFeatureState, async (ctx) => {
    ...
    const result = updateCharacterSessionFeatureState(config, campaign.slug, characterSlug, character.definition, ctx.req.param("featureKey") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionEquipment, async (ctx) => {
    ...
    const result = updateCharacterSessionEquipment(config, campaign.slug, characterSlug, character.definition, ctx.req.param("itemId") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionArtificerInfusions, async (ctx) => {
    ...
    const result = updateCharacterSessionArtificerInfusions(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.put(ROUTES.characterPortraitUpdate, async (ctx) => {
    ...
    const result = updateCharacterPortraitRevision(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
    ...
    const writeResult = await writeCampaignContentCharacterPortrait(config, campaign.slug, characterSlug, jsonPayload.payload, buildPreservedCharacterImportMetadata(campaign.slug, characterSlug, character.import_metadata));
    if (writeResult.status === "not_found") {
        const error = contentCharacterNotFound(campaign.slug, characterSlug);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.delete(ROUTES.characterPortraitDelete, async (ctx) => {
    ...
    const profile = typeof character.definition.profile === "object" &&
    ...
    const writeResult = await deleteCampaignContentCharacterPortrait(config, campaign.slug, characterSlug, buildPreservedCharacterImportMetadata(campaign.slug, characterSlug, character.import_metadata));
    if (writeResult.status === "not_found") {
        const error = contentCharacterNotFound(campaign.slug, characterSlug);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionXianxiaInventoryEquipped, async (ctx) => {
    ...
    const result = updateCharacterSessionXianxiaInventoryEquipped(config, campaign.slug, characterSlug, character.definition, ctx.req.param("itemId") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.post(ROUTES.characterSessionXianxiaInventoryAdd, async (ctx) => {
    ...
    const result = updateCharacterSessionXianxiaInventoryAdd(config, campaign.slug, characterSlug, character.definition, jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.delete(ROUTES.characterSessionXianxiaInventoryRemove, async (ctx) => {
    ...
    const result = updateCharacterSessionXianxiaInventoryRemove(config, campaign.slug, characterSlug, character.definition, ctx.req.param("itemId") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
...
app.patch(ROUTES.characterSessionXianxiaInventoryItem, async (ctx) => {
    ...
    const result = updateCharacterSessionXianxiaInventoryItem(config, campaign.slug, characterSlug, character.definition, ctx.req.param("itemId") || "", jsonPayload.payload, auth.actorUserId ?? 0);
    if (result.status === "state_conflict") {
        const error = stateConflict(result.message);
        return ctx.json({ ok: error.ok, error: error.error }, error.status);
    }
