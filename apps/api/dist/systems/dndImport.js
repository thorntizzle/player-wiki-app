...
const SUPPORTED_ENTRY_TYPE_SET = new Set(DND5E_SUPPORTED_ENTRY_TYPES);
...
function decodeEmbeddedArchive(payload) {
...
function normalizeSourceIds(value) {
    ...
    const sourceIds = Array.from(new Set(value.map((item) => String(item || "").trim().toUpperCase()).filter(Boolean)));
...
function normalizeArchiveMemberName(rawName) {
...
function readArchiveFiles(archiveBytes) {
    let rawFiles;
    try {
...
function resolveDataRootPrefix(files) {
    ...
    return {
        error: "Import archives must contain a compatible DND 5E source data/ directory at the root or inside one top-level folder.",
    };
}
function readJsonFile(files, dataRootPrefix, relativePath) {
...
function datasetPath(dataset, sourceId) {
...
function normalizeInlineTags(value) {
    return value.replace(/\{@([^{}]+)\}/g, (_match, body) => {
...
function renderEntriesHtml(value) {
    if (!value) {
        return "";
    }
    if (Array.isArray(value)) {
...
function identitySeed(entryType, rawEntry) {
...
function makeUniqueIdentifier(base, used, page) {
    if (!used.has(base)) {
        used.add(base);
        return base;
    }
...
function shouldSkipEntry(entryType, rawEntry) {
...
function isPlayerSafeDefault(sourceId, entryType) {
    return PLAYER_SAFE_SOURCE_IDS.has(sourceId) && PLAYER_SAFE_ENTRY_TYPES.has(entryType);
}
function isDmHeavy(sourceId, entryType) {
    return !isPlayerSafeDefault(sourceId, entryType);
}
function buildMetadata(entryType, rawEntry) {
    const metadata = {};
    for (const key of [
        "level",
        "school",
...
function buildRenderedHtml(entryType, body) {
    const sections = [];
    const labeledKeys = [
        ["entries", "Details"],
...
function buildEntry(librarySlug, entryType, rawEntry, sourcePath, usedEntryKeys, usedSlugs) {
    const title = String(rawEntry.name || "").trim();
    ...
    const entryKey = makeUniqueIdentifier(`${librarySlug.toLowerCase()}|${entryType}|${sourceId.toLowerCase()}|${seed}`, usedEntryKeys, rawEntry.page);
...
function rawEntriesForDataset(payload, dataset) {
    const record = asRecord(payload);
    const rawEntries = record[dataset.jsonKey];
    if (!Array.isArray(rawEntries)) {
        return [];
    }
    return rawEntries.filter((entry) => typeof entry === "object" && entry !== null && !Array.isArray(entry));
}
function ensureSupportedSource(database, librarySlug, sourceId) {
    const row = database
        .prepare("SELECT source_id FROM systems_sources WHERE library_slug = ? AND source_id = ?")
        .get(librarySlug, sourceId);
    return Boolean(row);
}
...
function loadImportRun(database, importRunId) {
    const row = database
        .prepare(`
        SELECT
          id,
...
function createImportRun(database, librarySlug, sourceId, importVersion, sourcePath, entryTypes, actorUserId) {
...
function completeImportRun(database, importRunId, summary) {
    database
        .prepare(`
...
function failImportRun(database, importRunId, summary) {
    database
        .prepare(`
        UPDATE systems_import_runs
        SET status = 'failed',
...
function replaceEntriesForSource(database, librarySlug, sourceId, entries, entryTypes) {
    const now = utcIsoTimestamp();
    ...
    const existingKeys = database
        ...
        `)
        .all(...params).map((row) => String(row.entry_key));
    ...
    for (const entry of entries) {
        insertEntry.run(librarySlug, sourceId, entry.entry_key, entry.entry_type, entry.slug, entry.title, entry.source_page, entry.source_path, entry.search_text, entry.player_safe_default ? 1 : 0, entry.dm_heavy ? 1 : 0, JSON.stringify(entry.metadata), JSON.stringify(entry.body), entry.rendered_html, now, now);
    }
    return entries.length;
}
function loadEntriesForSource(files, dataRootPrefix, librarySlug, sourceId, entryTypes) {
    const entries = [];
    ...
    for (const dataset of DATASETS) {
        ...
        const rawEntries = rawEntriesForDataset(payload, dataset).filter((entry) => String(entry.source || "").trim().toUpperCase() === sourceId);
...
function importOneSource(database, files, dataRootPrefix, payload, sourceId, actorUserId) {
    if (!sourceId) {
    ...
    const importRun = createImportRun(database, DND_5E_LIBRARY_SLUG, sourceId, payload.importVersion, payload.sourcePathLabel, entryTypes, actorUserId);
    const importedByType = {};
    const sourceFiles = [];
    try {
        let importedCount = 0;
...
export function importDnd5eSystemsArchive(dbPath, payload, actorUserId) {
    ...
    try {
        const importResults = normalizedPayload.sourceIds.map((sourceId) => importOneSource(database, files, dataRootPrefix, normalizedPayload, sourceId, actorUserId));
