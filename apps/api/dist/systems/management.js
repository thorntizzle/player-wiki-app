...
import { entryTypeLabel, entryTypeSortKey, isNoSuchTableError, loadCampaignEntryOverrides, loadEntriesForSources, loadSourceRows, parseJsonValue, parseMetadata, parseSourceSeeds, serializeEntrySummary, serializeLibrary, serializeSourceState, SYSTEMS_ENTRY_TYPE_LABELS, } from "./sources.js";
...
const XIANXIA_CUSTOM_ENTRY_FACETS = {
...
function canSetPrivateVisibility(role) {
    return role === "admin";
}
...
function normalizeVisibilityChoice(value) {
...
function defaultSystemsScopeVisibility(campaign) {
...
function compareEntryTypes(left, right) {
...
function loadLibrary(database, librarySlug) {
    if (!librarySlug) {
        return null;
    }
    try {
        return serializeLibrary(database
            .prepare(`
...
function loadPolicy(database, campaignSlug) {
...
function loadEffectiveSystemsScopeVisibility(database, campaign) {
    ...
    try {
        ...
        `)
            .all(campaign.slug);
        const visibilityByScope = new Map();
        ...
        return mostPrivateVisibility(visibilityByScope.get("campaign") || defaultCampaignVisibility, visibilityByScope.get("systems") || defaultSystemsVisibility);
    }
    catch (error) {
...
function sourceRowsForManagement(database, campaign, campaignConfig, role, librarySlug, systemsScopeVisibility) {
    if (!librarySlug) {
        return [];
    }
    ...
    try {
        ...
        return loadSourceRows(database, campaign.slug, librarySlug).map((row) => {
            ...
            const sourceVisibility = mostPrivateVisibility(systemsVisibility, sourceState.default_visibility);
            return {
                ...sourceState,
                permissions: {
...
function visibilityLabel(value, fallback = "") {
    const normalized = String(value || "").trim();
    return normalized ? VISIBILITY_LABELS[normalized] || normalized : fallback;
}
function effectiveEntryVisibility(entry, sourceState, override) {
...
function canAccessEntryLink(role, systemsScopeVisibility, entry, sourceState, override) {
    const overrideEnablement = override?.is_enabled_override;
    ...
    const visibility = mostPrivateVisibility(normalizeVisibilityChoice(systemsScopeVisibility) || "private", effectiveEntryVisibility(entry, sourceState, override));
    if (role === "dm") {
        return visibility === "public" || visibility === "players" || visibility === "dm";
    }
    return visibility === "public" || visibility === "players";
}
function serializeOverrideRow(campaignSlug, role, systemsScopeVisibility, override, entry, sourceState) {
    ...
    return {
        ...
        entry_href: entrySlug && canAccessEntryLink(role, systemsScopeVisibility, entry, sourceState, override)
...
function entryByKey(rows) {
    return new Map(rows.map((row) => [row.entry_key, row]));
}
function sourceStateById(rows) {
    return new Map(rows.map((row) => [row.source_id, row]));
}
function loadEntryOverrideRows(database, campaignSlug, librarySlug, sourceRows, role, systemsScopeVisibility) {
    if (!librarySlug) {
        return [];
    }
    try {
        const overrides = [...loadCampaignEntryOverrides(database, campaignSlug, librarySlug).values()];
        const entries = entryByKey(loadEntriesForSources(database, librarySlug, sourceRows.map((source) => source.source_id)));
...
function campaignItemReviewForMetadata(metadata) {
    const review = metadata.campaign_item_mechanics;
    if (typeof review === "object" && review !== null && !Array.isArray(review)) {
        return { ...review };
    }
    const hasReviewMetadata = Object.hasOwn(metadata, "campaign_item_mechanics") ||
...
function customEntryDefaultVisibility(campaign) {
    const system = String(campaign.system || "").trim().toLowerCase();
...
function isCampaignCustomEntry(campaignSlug, metadata) {
...
function serializeCustomEntry(campaign, role, systemsScopeVisibility, entry, override, sourceState) {
...
function loadCustomEntrySourceRows(database, campaign, librarySlug, sourceRows, role, systemsScopeVisibility) {
    if (!librarySlug) {
    ...
    try {
        ...
        const rows = customSourceRows.map((sourceState) => {
            const entries = loadEntriesForSources(database, librarySlug, [sourceState.source_id])
                ...
                .map((entry) => serializeCustomEntry(campaign, role, systemsScopeVisibility, entry, overrides.get(entry.entry_key), sourceState));
...
function linkedCustomItemEntries(database, campaign, librarySlug, sourceRows) {
...
function loadCampaignItemPageRows(database, campaign, librarySlug, sourceRows) {
...
function customCampaignSourceId(campaignSlug) {
...
function normalizeCustomSlugLeaf(value) {
...
function renderCustomEntryMarkdown(markdownText) {
...
function normalizeCampaignItemReviewStatus(value) {
...
function buildCampaignItemMechanicsMetadata(title, sourcePageRef, reviewStatus, explicitMechanics, sourcePageMetadata) {
    const mechanics = asRecord(explicitMechanics);
    const normalizedReview = normalizeCampaignItemReviewStatus(reviewStatus || mechanics.review_status || mechanics.campaign_item_mechanics_review_status);
...
function stampXianxiaCustomEntryMetadata(metadata, body, entryType, slug, title) {
...
function loadEntryBySlug(database, librarySlug, entrySlug) {
    return (database
        .prepare(`
          SELECT
            id,
          ...
          WHERE library_slug = ?
            AND slug = ?
        `)
        .get(librarySlug, entrySlug) || null);
}
function loadCampaignEnabledSource(database, campaignSlug, sourceId) {
    return database
        .prepare(`
        SELECT source_id
        ...
        WHERE campaign_slug = ?
          AND source_id = ?
      `)
        .get(campaignSlug, sourceId);
}
function upsertCampaignSystemsPolicyForCustomEntry(database, campaignSlug, librarySlug, actorUserId, now) {
    database
        .prepare(`
...
function upsertCustomCampaignSource(database, librarySlug, sourceId, title, now) {
    database
        .prepare(`
...
function insertCampaignEnabledSourceIfMissing(database, campaign, librarySlug, sourceId, actorUserId, now) {
...
function upsertCustomSystemsEntryRow(database, { librarySlug, sourceId, entryKey, entryType, slug, title, provenance, searchText, visibility, metadata, body, renderedHtml, now, }) {
    ...
    database
        ...
        ON CONFLICT(library_slug, entry_key) DO UPDATE SET
          ...
          updated_at = excluded.updated_at
      `)
        .run(librarySlug, sourceId, entryKey, entryType, slug, title, provenance, searchText, visibility === "public" || visibility === "players" ? 1 : 0, visibility === "dm" || visibility === "private" ? 1 : 0, JSON.stringify(metadata), JSON.stringify(body), renderedHtml, existing?.created_at || now, now);
}
function upsertCampaignEntryOverrideForCustomEntry(database, campaignSlug, librarySlug, entryKey, visibilityOverride, isEnabledOverride, actorUserId, now) {
    database
        .prepare(`
...
function insertCustomEntryAuditEvent(database, actorUserId, campaignSlug, eventType, entry, now) {
    database
        .prepare(`
        ...
        VALUES (?, NULL, ?, NULL, ?, ?, ?)
      `)
        .run(actorUserId, campaignSlug, eventType, JSON.stringify({
...
function insertItemMechanicsImportAuditEvent(database, actorUserId, campaignSlug, entry, pageRef, now) {
    database
        .prepare(`
        INSERT INTO auth_audit_log (
          actor_user_id,
        ...
        VALUES (?, NULL, ?, NULL, ?, ?, ?)
      `)
        .run(actorUserId, campaignSlug, "campaign_systems_item_mechanics_imported", JSON.stringify({
        entry_key: entry.entry_key,
        entry_slug: entry.slug,
...
function loadPublishedItemPage(database, campaignSlug, pageRef) {
    if (!tableExists(database, "campaign_pages")) {
        return null;
    }
    return (database
        .prepare(`
          ...
          WHERE campaign_slug = ?
            ...
            AND published != 0
        `)
        .get(campaignSlug, pageRef) || null);
}
function loadCustomItemEntryByLinkedPage(database, campaign, librarySlug, pageRef) {
...
function customSourceState(database, campaign, campaignConfig, role, librarySlug, sourceId) {
    const seeds = parseSourceSeeds(campaignConfig);
...
function existingCustomEntryContext(database, campaign, campaignConfig, role, librarySlug, entrySlug, invalidMessage) {
    const entry = loadEntryBySlug(database, librarySlug, String(entrySlug || "").trim());
    if (!entry) {
        return { status: "validation_error", message: invalidMessage };
    }
    ...
    return { status: "ok", entry, sourceState };
}
function ensureCustomCampaignSource(database, campaign, campaignConfig, role, librarySlug, actorUserId, now) {
    ...
    upsertCustomCampaignSource(database, librarySlug, sourceId, `${campaign.title || campaign.slug} Custom Systems`, now);
    ...
    if (!sourceState) {
        return { status: "validation_error", message: "The custom Systems source is no longer available." };
    }
    return { status: "ok", sourceId, sourceState };
}
function prepareCustomEntryWrite(database, campaign, librarySlug, sourceState, sourceId, actorUserId, role, payload, existingEntry) {
    const normalizedTitle = String(payload.title || "").trim();
    if (!normalizedTitle) {
        return { status: "validation_error", message: "Custom Systems entries need a title." };
    }
    if (normalizedTitle.length > 200) {
        return { status: "validation_error", message: "Custom Systems entry titles must stay under 200 characters." };
    }
    const normalizedEntryType = normalizeEntryType(payload.entry_type);
    if (!normalizedEntryType) {
        return { status: "validation_error", message: "Choose an entry type before saving this custom Systems entry." };
    }
    const requestedVisibility = normalizeVisibilityChoice(payload.visibility) ||
    ...
    if (normalizedProvenance.length > 500) {
        return { status: "validation_error", message: "Custom Systems provenance must stay under 500 characters." };
    }
    ...
    let sourcePageMetadata = {};
    if (normalizedSourcePageRef) {
        if (normalizedSourcePageRef.length > 500) {
            return { status: "validation_error", message: "Linked item page references must stay under 500 characters." };
        }
        if (normalizedEntryType !== "item") {
            return { status: "validation_error", message: "Only item custom Systems entries can link item mechanics pages." };
        }
        const linkedPage = loadPublishedItemPage(database, campaign.slug, normalizedSourcePageRef);
        if (!linkedPage) {
            return { status: "validation_error", message: "Choose a valid published item page before saving item mechanics." };
        }
    ...
    if (!normalizedBodyMarkdown) {
        return { status: "validation_error", message: "Custom Systems entries need a rendered body." };
    }
    if (normalizedBodyMarkdown.length > 100_000) {
        return { status: "validation_error", message: "Custom Systems entry bodies must stay under 100,000 characters." };
    }
    ...
    if (!existingEntry && !normalizedSlugLeaf) {
        return { status: "validation_error", message: "Choose a URL slug or title before saving a custom Systems entry." };
    }
    ...
    if (normalizedEntryType === "item") {
        const itemMetadata = buildCampaignItemMechanicsMetadata(normalizedTitle, normalizedSourcePageRef, payload.item_mechanics_review_status || payload.mechanics_review_status, payload.item_mechanics, sourcePageMetadata);
...
function serializeMutatedCustomEntry(database, campaign, role, systemsScopeVisibility, entry, override, sourceState) {
...
function buildCustomEntryMutationResponse(dbPath, campaign, campaignConfig, role, entry) {
    return {
        status: "ok",
        entry,
...
function customEntryMutationErrorMessage(error) {
    if (isNoSuchTableError(error)) {
        return "Systems database is unavailable.";
    }
    return error instanceof Error ? error.message : String(error);
}
export function createCustomSystemsEntry(dbPath, campaign, campaignConfig, role, actorUserId, payload) {
    ...
    try {
        ...
        const writeChanges = database.transaction(() => {
            ...
            const prepared = prepareCustomEntryWrite(database, campaign, librarySlug, source.sourceState, source.sourceId, actorUserId, role, payload, null);
            ...
            const override = upsertCampaignEntryOverrideForCustomEntry(database, campaign.slug, librarySlug, entry.entry_key, prepared.visibility, null, actorUserId, now);
...
export function importCampaignItemMechanics(dbPath, campaign, campaignConfig, role, actorUserId, payload) {
    if (!existsSync(dbPath)) {
        return { status: "validation_error", message: "Systems database is unavailable." };
    }
    const librarySlug = String(campaign.systems_library_slug || "").trim();
    ...
    try {
        ...
        const writeChanges = database.transaction(() => {
            ...
            const existingOverride = existingEntry
                ...
                : undefined;
            const requestedVisibility = String(payload.visibility || "").trim() ||
                existingOverride?.visibility_override ||
                customEntryDefaultVisibility(campaign);
            const pageSourceRef = String(page.source_ref || "").trim();
            const prepared = prepareCustomEntryWrite(database, campaign, librarySlug, source.sourceState, source.sourceId, actorUserId, role, {
                title,
                entry_type: "item",
            ...
            const override = upsertCampaignEntryOverrideForCustomEntry(database, campaign.slug, librarySlug, entry.entry_key, prepared.visibility, existingOverride?.is_enabled_override ?? null, actorUserId, now);
...
export function updateCustomSystemsEntry(dbPath, campaign, campaignConfig, role, actorUserId, entrySlug, payload) {
    if (!existsSync(dbPath)) {
        return { status: "validation_error", message: "Systems database is unavailable." };
    }
    const librarySlug = String(campaign.systems_library_slug || "").trim();
    ...
    const database = new Database(dbPath, { fileMustExist: true });
    try {
        const context = existingCustomEntryContext(database, campaign, campaignConfig, role, librarySlug, entrySlug, "Choose a valid custom Systems entry before saving.");
        ...
        const writeChanges = database.transaction(() => {
            upsertCampaignSystemsPolicyForCustomEntry(database, campaign.slug, librarySlug, actorUserId, now);
            const prepared = prepareCustomEntryWrite(database, campaign, librarySlug, context.sourceState, context.entry.source_id, actorUserId, role, payload, context.entry);
            if (prepared.status === "validation_error") {
                throw new Error(prepared.message);
            }
...
function setCustomSystemsEntryArchivedState(dbPath, campaign, campaignConfig, role, actorUserId, entrySlug, archived) {
    if (!existsSync(dbPath)) {
        return { status: "validation_error", message: "Systems database is unavailable." };
    }
    const librarySlug = String(campaign.systems_library_slug || "").trim();
    ...
    try {
        ...
        const writeChanges = database.transaction(() => {
            upsertCampaignSystemsPolicyForCustomEntry(database, campaign.slug, librarySlug, actorUserId, now);
            const existingOverride = loadCampaignEntryOverrides(database, campaign.slug, librarySlug).get(context.entry.entry_key);
            const override = upsertCampaignEntryOverrideForCustomEntry(database, campaign.slug, librarySlug, context.entry.entry_key, existingOverride?.visibility_override ?? null, archived ? false : null, actorUserId, now);
            insertCustomEntryAuditEvent(database, actorUserId, campaign.slug, archived ? "campaign_systems_custom_entry_archived" : "campaign_systems_custom_entry_restored", context.entry, now);
            const systemsScopeVisibility = loadEffectiveSystemsScopeVisibility(database, campaign);
            serializedEntry = serializeMutatedCustomEntry(database, campaign, role, systemsScopeVisibility, context.entry, override, context.sourceState);
        });
        writeChanges();
        return buildCustomEntryMutationResponse(dbPath, campaign, campaignConfig, role, serializedEntry);
    }
...
export function archiveCustomSystemsEntry(dbPath, campaign, campaignConfig, role, actorUserId, entrySlug) {
    return setCustomSystemsEntryArchivedState(dbPath, campaign, campaignConfig, role, actorUserId, entrySlug, true);
}
export function restoreCustomSystemsEntry(dbPath, campaign, campaignConfig, role, actorUserId, entrySlug) {
...
function loadImportRunRows(database, librarySlug) {
...
export function buildDmContentSystemsPayload(dbPath, campaign, campaignConfig, role) {
    ...
    try {
        ...
        const sourceRowsForPayload = sourceRows.map((source) => source);
        const importRunRows = loadImportRunRows(database, librarySlug);
        const entryOverrideRows = loadEntryOverrideRows(database, campaign.slug, librarySlug, sourceRowsForPayload, role, systemsScopeVisibility);
        const customEntries = loadCustomEntrySourceRows(database, campaign, librarySlug, sourceRowsForPayload, role, systemsScopeVisibility);
        return {
            ...baseline,
            library,
