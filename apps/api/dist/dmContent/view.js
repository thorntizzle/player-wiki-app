...
function canManageDmContent(role) {
...
function extractStatblockTitleHeading(markdownText) {
...
function isGenericStatblockHeading(value) {
...
function searchInt(pattern, value) {
...
function extractStatblockDexterityModifier(markdownText, fallback = 0) {
...
function normalizeStatblockSubsection(value) {
    const normalized = String(value || "").trim();
...
function buildParserFeedback(row) {
...
function parseStatblockMarkdownUpload(options) {
    ...
    const normalizedSubsection = normalizeStatblockSubsection(options.subsectionHint || metadata.subsection || metadata.group || metadata.section);
...
function listStatblocks(database, campaignSlug) {
    try {
        const rows = database
            .prepare(`SELECT *
...
function getStatblock(database, campaignSlug, statblockId) {
    const row = database
        .prepare("SELECT * FROM campaign_dm_statblocks WHERE campaign_slug = ? AND id = ?")
        .get(campaignSlug, statblockId);
    return row ? serializeStatblock(row) : null;
}
function getStatblockRow(database, campaignSlug, statblockId) {
    return database
        .prepare("SELECT * FROM campaign_dm_statblocks WHERE campaign_slug = ? AND id = ?")
        .get(campaignSlug, statblockId);
}
function listConditions(database, campaignSlug) {
    try {
        const rows = database
            .prepare(`SELECT *
...
function getCondition(database, campaignSlug, conditionDefinitionId) {
    const row = database
        .prepare("SELECT * FROM campaign_dm_condition_definitions WHERE campaign_slug = ? AND id = ?")
        .get(campaignSlug, conditionDefinitionId);
    return row ? serializeCondition(row) : null;
}
function normalizeConditionPayload(database, campaignSlug, options) {
    const normalizedName = options.name === undefined ? String(options.existing?.name || "") : String(options.name || "").trim();
    if (!normalizedName) {
    ...
    const normalizedDescription = options.description_markdown === undefined
...
function countStagedArticles(database, campaignSlug) {
    try {
        const row = database
...
function countSystemsSources(database, campaign) {
...
export function createDmContentStatblock(dbPath, campaignSlug, payload, actorUserId) {
    let upload;
    try {
    ...
    try {
        ...
        const result = database
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `)
            .run(campaignSlug, upload.title, upload.body_markdown, upload.source_filename, upload.subsection, upload.armor_class, upload.max_hp, upload.speed_text, upload.movement_total, upload.initiative_bonus, now, now, actorUserId, actorUserId);
...
export function updateDmContentStatblock(dbPath, campaignSlug, statblockId, payload, actorUserId) {
    ...
    try {
        ...
        `)
            .run(upload.title, upload.body_markdown, upload.subsection, upload.armor_class, upload.max_hp, upload.speed_text, upload.movement_total, upload.initiative_bonus, now, actorUserId, campaignSlug, statblockId);
        if (result.changes === 0) {
            return { status: "validation_error", message: "That statblock could not be found." };
        }
...
export function deleteDmContentStatblock(dbPath, campaignSlug, statblockId) {
    const database = new Database(dbPath, { fileMustExist: true });
    try {
        const statblock = getStatblock(database, campaignSlug, statblockId);
        if (!statblock) {
...
export function createDmContentCondition(dbPath, campaignSlug, payload, actorUserId) {
    const database = new Database(dbPath, { fileMustExist: true });
    try {
        ...
        const result = database
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `)
            .run(campaignSlug, conditionPayload.name, conditionPayload.description_markdown, now, now, actorUserId, actorUserId);
...
export function updateDmContentCondition(dbPath, campaignSlug, conditionDefinitionId, payload, actorUserId) {
    ...
    try {
        ...
        const result = database
          ...
          WHERE campaign_slug = ? AND id = ?
        `)
            .run(conditionPayload.name, conditionPayload.description_markdown, now, actorUserId, campaignSlug, conditionDefinitionId);
        if (result.changes === 0) {
            return { status: "validation_error", message: "That custom condition could not be found." };
        }
...
export function deleteDmContentCondition(dbPath, campaignSlug, conditionDefinitionId) {
    const database = new Database(dbPath, { fileMustExist: true });
    try {
        const condition = getCondition(database, campaignSlug, conditionDefinitionId);
        if (!condition) {
...
export function buildDmContentPayload(dbPath, campaign, role, playerWikiPageCount) {
    let statblocks = [];
    let conditions = [];
