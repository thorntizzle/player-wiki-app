...
function normalizeRemovalSafety(record, removalSafety) {
...
function withVisibilityDefaults(record) {
    return {
...
export function buildCampaignConfigPayload(record) {
    return {
        ok: true,
        config_file: {
...
function profileClassLevelText(profile, fallback = "Character") {
...
function summarizeResourceValue(resource) {
...
function buildResourcePreview(state) {
...
function isXianxiaSystem(value) {
...
function profileClassRows(profile) {
...
function deriveHitDiceMaxPools(definition) {
...
function buildHitDiceSummary(definition, state) {
...
function buildCharacterPortraitPayload(campaignSlug, record, assetByRef) {
...
export function buildCharacterDetailPayload({ campaign, record, stateRecord, assetByRef, permissions, controls, }) {
...
export function buildCharacterRosterPayload({ campaign, records, stateBySlug, assetByRef, query, canManageSession, }) {
...
function buildContentCharacterSummaryPayload(record) {
    return {
        character_slug: record.character_slug,
        updated_at: record.updated_at,
...
export function buildContentCharacterListPayload(records) {
    return {
        ok: true,
        characters: records.map(buildContentCharacterSummaryPayload),
    };
}
export function buildContentCharacterDetailPayload(record) {
    return {
        ok: true,
        character_file: {
            character_slug: record.character_slug,
...
export function buildContentCharacterDeletePayload(deleted) {
    return {
        ok: true,
        deleted,
    };
}
function buildContentAssetFilePayload(campaignSlug, record, includeData) {
    const payload = {
...
export function buildContentAssetListPayload(campaignSlug, records) {
    return {
        ok: true,
        assets: records.map((record) => buildContentAssetFilePayload(campaignSlug, record, false)),
    };
}
export function buildContentAssetDetailPayload(campaignSlug, record) {
    return {
        ok: true,
        asset_file: buildContentAssetFilePayload(campaignSlug, record, true),
    };
}
export function buildContentAssetWritePayload(campaignSlug, record) {
    return {
        ok: true,
        asset_file: buildContentAssetFilePayload(campaignSlug, record, false),
    };
}
export function buildContentAssetDeletePayload(record) {
    return {
        ok: true,
        deleted: {
            asset_ref: record.asset_ref,
...
function buildContentPageFilePayload(record, includeBody, removalSafety) {
...
export function buildContentPageListPayload(records, removalSafety) {
    return {
        ok: true,
        pages: records.map((record) => buildContentPageFilePayload(record, false, removalSafety?.[record.page_ref])),
    };
}
export function buildContentPageDetailPayload(record, removalSafety) {
...
export function buildContentPageDeletePayload(record) {
    return {
        ok: true,
        deleted: {
            page_ref: record.page_ref,
