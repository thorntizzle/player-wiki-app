...
interface HelpSurface {
  ...
  limits: string[];
  links: HelpLink[];
  guidance_cards: HelpGuidanceCard[];
}

interface HelpVisibilityRow {
  label: string;
  visibility_label: string;
  viewer_can_open: boolean;
}

export interface CampaignHelpPayload {
  ok: true;
  ...
  visibility_rows: HelpVisibilityRow[];
...
function campaignHomeSurface(campaignSlug: string): HelpSurface {
  return {
...
export function buildCampaignHelpPayload(campaign: CampaignViewModel): CampaignHelpPayload {
  ...
  return {
    ...
    viewer_role_summary:
      "You are viewing the public portion of this campaign. Member-only surfaces stay hidden until you sign in with the right campaign access.",
    ...
    surfaces,
    account_note:
      "Sign in to save theme preferences, choose a live Session chat order, and open member-only surfaces.",
    links: {
