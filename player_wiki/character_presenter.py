...
from .character_mechanics_projection import (
    build_armorer_combat_availability,
...
DND_SECTION_DEPENDENCY_MANIFEST = MappingProxyType(
    {
        "overview": DndSectionDependency(
            ...
            derivation_components=_DND_ITEM_ABILITY_DERIVATION_COMPONENTS,
        ),
...
def _present_character_detail(
    ...
    *,
        ...
        projection_kwargs.update(
            {
                ...
                "derivation_components": scope_dependency.derivation_components,
            }
        )
    ...
    xianxia_effort_damage = xianxia_projection.get("effort_damage") if is_xianxia_character else None
    xianxia_check_formula = xianxia_projection.get("check_formula") if is_xianxia_character else None
    xianxia_difficulty_states = xianxia_projection.get("difficulty_states") if is_xianxia_character else None
    xianxia_honor_interactions = xianxia_projection.get("honor_interactions") if is_xianxia_character else None
    xianxia_skill_use_guardrails = xianxia_projection.get("skill_use_guardrails") if is_xianxia_character else None
    xianxia_rule_text_references = (
        list(xianxia_projection.get("rule_text_references") or [])
        if is_xianxia_character
        else []
    )
    xianxia_active_state_reminders = (
        list(xianxia_projection.get("active_state_reminders") or [])
        if is_xianxia_character
        else []
    )
    xianxia_stance_break = xianxia_projection.get("stance_break") if is_xianxia_character else None
    ...
    if build_spellcasting and (spellcasting_payload.get("spells") or slot_lanes):
        ...
        for spell in list(spellcasting_payload.get("spells") or []):
            ...
            spell_action_state = project_spell_action_state(
                spell=dict(spell or {}),
            ...
            presented_spell["remove_label"] = _presented_spell_remove_label(
                mode=row_mode,
                is_cantrip=bool(spell_action_state.get("is_cantrip")),
                is_prepared=bool(spell_action_state.get("is_prepared")),
            )
    ...
    for projected_attack in (
        ...
        if bool(projected_attack.get("hidden")):
        ...
        attacks.append(
            {
                ...
                "linked_item_refs": list(projected_attack.get("linked_item_refs") or []),
                "is_equipped": projected_attack.get("is_equipped"),
            }
        )
...
def present_xianxia_read_context(
    ...
    *,
    ...
    active_stance = coerce_xianxia_active_state_record(xianxia_state.get("active_stance"))
    active_aura = coerce_xianxia_active_state_record(xianxia_state.get("active_aura"))
