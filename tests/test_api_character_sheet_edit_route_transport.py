...
def test_transport_has_exact_dependency_registration_and_composition_shape() -> None:
    ...
    assert isinstance(register_api.body[227], ast.FunctionDef)
    assert register_api.body[227].name == "run_character_definition_mutation"
    assert isinstance(register_api.body[228], ast.Expr)
    assert register_api.body[228].value.func.id == (
    ...
    dependency_call = next(
        node
        for node in ast.walk(register_api.body[228])
        if isinstance(node, ast.Call)
