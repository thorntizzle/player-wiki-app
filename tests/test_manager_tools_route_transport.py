...
SESSION_READINESS_ROUTE = (
    "/campaigns/<campaign_slug>/manager-tools/session-readiness"
)
SESSION_READINESS_URL = "/campaigns/linden-pass/manager-tools/session-readiness"
...
def test_manager_tools_registrar_owns_two_get_only_rules_without_decorators() -> None:
    register = _function("register_manager_tools_routes")
    handlers = (
        _function("campaign_manager_tools_view"),
        _function("campaign_session_readiness_view"),
    )
    registrations = [
        node
    ...
    for registration in registrations:
        ...
        records.append(
            (
                ast.literal_eval(registration.args[0]),
                ast.literal_eval(keywords["endpoint"]),
                ast.literal_eval(keywords["methods"]),
            )
        )
    assert records == [
        (MANAGER_TOOLS_ROUTE, "campaign_manager_tools_view", ("GET",)),
        (
            SESSION_READINESS_ROUTE,
...
@pytest.mark.parametrize("method", ("POST", "PUT", "PATCH", "DELETE"))
def test_session_readiness_write_methods_are_405_and_private_no_store(
    client,
    sign_in,
    users,
    method,
    ...
    response = client.open(SESSION_READINESS_URL, method=method)
...
def test_session_readiness_get_head_options_keep_implicit_transport_and_protection(
    client,
    sign_in,
    users,
) -> None:
    ...
    get_response = client.get(SESSION_READINESS_URL)
    head_response = client.head(SESSION_READINESS_URL)
    options_response = client.open(SESSION_READINESS_URL, method="OPTIONS")
    ...
    missing = client.get(
        "/campaigns/no-such-campaign/manager-tools/session-readiness"
    )
    assert missing.status_code == 404
    assert missing.headers["Cache-Control"] == "private, no-store"
    assert missing.headers["Referrer-Policy"] == "no-referrer"
