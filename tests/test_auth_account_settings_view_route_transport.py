...
def test_transport_has_exact_direct_dependencies_registration_and_source_shape() -> None:
    ...
    assert register_auth.body[9].name == "account_theme_update"
...
def test_moved_handler_keeps_canonical_ast_and_every_unrelated_auth_identity() -> None:
    ...
    old_unrelated = [
        node for index, node in enumerate(old_register.body) if index != 8
    ]
    new_unrelated = [
        node for index, node in enumerate(new_register.body) if index != 8
    ]
