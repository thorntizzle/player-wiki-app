...
def test_transport_has_exact_dependency_registration_and_composition_shape() -> None:
    ...
    assert sum(isinstance(node, ast.FunctionDef) for node in register_api.body) == 238
    assert sum(isinstance(node, ast.FunctionDef) for node in ast.walk(register_api)) == 252
    ...
    assert len(api_route_decorators) == 70
    ...
    assert isinstance(register_api.body[244], ast.FunctionDef)
    assert register_api.body[244].name == "character_item_action_use"
