...
def test_transport_has_exact_dependency_registration_and_late_binding_shape() -> None:
    ...
    assert sum(isinstance(node, ast.FunctionDef) for node in register_api.body) == 240
    assert sum(isinstance(node, ast.FunctionDef) for node in ast.walk(register_api)) == 254
    ...
    assert len(api_route_decorators) == 72
    ...
    assert isinstance(register_api.body[242], ast.FunctionDef)
    assert register_api.body[242].name == "character_resource_update"
