...
def test_route_registration_sources_match_the_checked_inventory() -> None:
    expected = {
        ...
        "character_xianxia_inventory_item_remove_api_routes.py": 0,
        "character_xianxia_inventory_equipped_update_api_routes.py": 0,
        "character_equipment_state_api_routes.py": 0,
        "character_artificer_infusions_api_routes.py": 0,
...
def test_auth_me_view_as_update_api_route_keeps_contract_and_module_ownership() -> None:
    endpoint = "api.me_view_as_update"
    rules = discover_rules()
    matches = [rule for rule in rules if rule.endpoint == endpoint]
    assert len(matches) == 1
    assert matches[0].rule == "/api/v1/me/view-as"
    ...
    handler = module_function(
        "auth_me_view_as_update_api_routes.py", "me_view_as_update"
    )
    assert handler.decorator_list == []
    registrar = module_function(
        "auth_me_view_as_update_api_routes.py",
...
def test_auth_me_view_as_clear_api_route_keeps_contract_and_module_ownership() -> None:
    ...
    handler = module_function(
        "auth_me_view_as_clear_api_routes.py", "me_view_as_clear"
    )
    assert handler.decorator_list == []
    registrar = module_function(
        "auth_me_view_as_clear_api_routes.py",
...
def test_auth_me_settings_view_api_route_keeps_contract_and_module_ownership() -> None:
    ...
    handler = module_function(
        "auth_me_settings_view_api_routes.py", "me_settings"
    )
    assert handler.decorator_list == []
    registrar = module_function(
        "auth_me_settings_view_api_routes.py",
...
def test_auth_sign_out_route_keeps_contract_and_module_ownership() -> None:
    ...
    matches = [rule for rule in rules if rule.endpoint == endpoint]
    assert len(matches) == 1
    assert matches[0].rule == "/sign-out"
    assert explicit_methods(matches[0]) == ["POST"]
    assert set(matches[0].methods) == {"POST", "OPTIONS"}
    ...
    handler = module_function("auth_sign_out_routes.py", endpoint)
    assert handler.decorator_list == []
    registrar = module_function(
        "auth_sign_out_routes.py", "register_auth_sign_out_route"
    )
    registrations = [
        node
        for node in ast.walk(registrar)
...
def test_auth_account_settings_view_route_keeps_contract_and_module_ownership() -> None:
    endpoint = "account_settings_view"
    rules = discover_rules()
    matches = [rule for rule in rules if rule.endpoint == endpoint]
    assert len(matches) == 1
    assert matches[0].rule == "/account"
    assert explicit_methods(matches[0]) == ["GET"]
    assert set(matches[0].methods) == {"GET", "HEAD", "OPTIONS"}
    ...
    handler = module_function("auth_account_settings_view_routes.py", endpoint)
    assert handler.decorator_list == []
    registrar = module_function(
        "auth_account_settings_view_routes.py",
...
def test_auth_account_theme_route_keeps_contract_and_module_ownership() -> None:
    endpoint = "account_theme_update"
    rules = discover_rules()
    matches = [rule for rule in rules if rule.endpoint == endpoint]
    assert len(matches) == 1
    assert matches[0].rule == "/account/theme"
    assert explicit_methods(matches[0]) == ["POST"]
    assert set(matches[0].methods) == {"POST", "OPTIONS"}
    ...
    handler = module_function("auth_account_theme_routes.py", endpoint)
    assert handler.decorator_list == []
    registrar = module_function(
...
def test_character_xianxia_inventory_item_remove_api_route_keeps_contract_and_module_ownership() -> None:
...
def test_character_xianxia_inventory_equipped_update_api_route_keeps_contract_and_module_ownership() -> None:
...
def test_character_equipment_state_api_route_keeps_contract_and_module_ownership() -> None:
...
def test_character_artificer_infusions_api_route_keeps_contract_and_module_ownership() -> None:
...
def test_character_feature_state_api_route_keeps_contract_and_module_ownership() -> None:
...
def test_character_currency_api_route_keeps_contract_and_module_ownership() -> None:
    ...
    assert not any(
        isinstance(node, ast.FunctionDef)
        and node.name == "character_currency_update"
        for node in ast.walk(api_tree)
    )
    handler = module_function(
        "character_currency_api_routes.py",
...
def test_character_notes_api_route_keeps_contract_and_module_ownership() -> None:
    ...
    assert not any(
        isinstance(node, ast.FunctionDef)
        and node.name == "character_notes_update"
        for node in ast.walk(api_tree)
    )
    handler = module_function(
        "character_notes_api_routes.py",
...
def test_character_rest_api_route_keeps_contract_and_module_ownership() -> None:
    endpoint = "api.character_rest_apply"
    matches = [rule for rule in discover_rules() if rule.endpoint == endpoint]
    assert len(matches) == 1
    assert matches[0].rule == (
        "/api/v1/campaigns/<campaign_slug>/characters/<character_slug>/"
        "session/rest/<rest_type>"
    )
    assert explicit_methods(matches[0]) == ["POST"]
    assert set(matches[0].methods) == {"POST", "OPTIONS"}
...
def test_character_xianxia_active_state_api_route_keeps_contract_and_module_ownership() -> None:
...
def test_character_xianxia_dao_request_api_route_keeps_contract_and_module_ownership() -> None:
...
def test_combat_extracted_routes_keep_legacy_contract_and_module_ownership() -> None:
    ...
    expected_posts = {
        ...
        "campaign_combat_update_npc_resource":
    ...
    assert len(combat_browser_entries) == 35
    assert sum(entry["method"] == "GET" for entry in combat_browser_entries) == 9
    assert sum(entry["method"] == "POST" for entry in combat_browser_entries) == 26
...
def test_combat_condition_api_routes_keep_contract_and_module_ownership() -> None:
    ...
    assert sum(not entry["endpoint"].startswith("api.") for entry in combat_entries) == 35
