...
def test_transport_has_exact_dependency_registration_and_composition_shape() -> None:
    ...
    assert sum(isinstance(node, ast.FunctionDef) for node in create_app.body) == 208
    assert sum(isinstance(node, ast.FunctionDef) for node in ast.walk(create_app)) == 222
    route_decorators = [
        decorator
    ...
    assert len(route_decorators) == 38
    ...
    assert isinstance(create_app.body[287], ast.FunctionDef)
    assert create_app.body[287].name == "character_session_item_action_use"
