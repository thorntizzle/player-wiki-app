...
def test_second_init_is_true_no_op_without_write_or_backup(tmp_path):
    ...
    assert result.from_version == result.to_version == 7
...
def test_read_only_ledger_inspector_reports_current_without_transaction_or_write(tmp_path):
    ...
    assert inspection.applied_version == inspection.current_version == 7
...
def test_read_only_ledger_inspector_reports_missing_without_creating_it(tmp_path):
    ...
    assert inspection.current_version == 7
...
def test_concurrent_legacy_wal_initializers_serialize_snapshot_and_migration(tmp_path, repetition):
    ...
    assert [result["applied"] for result in results].count([1, 2, 3, 4, 5, 6, 7]) == 1
...
def test_v1_through_v6_payloads_remain_immutable_while_v7_owns_current_schema():
    ...
    assert MIGRATIONS[6].payload.schema_sql == CURRENT_SCHEMA_SQL == SCHEMA
    assert "content_api_update" in CURRENT_SCHEMA_SQL
...
def test_v1_database_advances_through_v2_to_v7_and_then_is_a_true_no_op(tmp_path):
    database_path = tmp_path / "v1-to-v7.sqlite3"
    with _connect(database_path) as connection:
...
def test_v4_active_character_row_migrates_through_v5_to_v7_losslessly(tmp_path):
...
def test_v6_active_character_rows_migrate_to_v7_without_field_or_blob_changes(tmp_path):
