...
def test_transport_has_exact_dependency_registration_and_composition_shape() -> None:
    ...
    assert isinstance(register_api.body[246], ast.Expr)
    assert register_api.body[246].value.func.id == (
        "register_character_xianxia_inventory_item_update_api_route"
    )
    assert isinstance(register_api.body[247], ast.Expr)
    assert register_api.body[247].value.func.id == (
        "register_character_xianxia_inventory_item_remove_api_route"
    )
    assert isinstance(register_api.body[248], ast.Expr)
    assert register_api.body[248].value.func.id == (
        "register_character_xianxia_inventory_equipped_update_api_route"
    )
    assert isinstance(register_api.body[249], ast.Expr)
    assert register_api.body[249].value.func.id == (
        "register_character_equipment_state_api_route"
    )
    assert isinstance(register_api.body[235], ast.FunctionDef)
    assert register_api.body[235].name == "xianxia_inventory_item_payload"

    dependency_call = next(
        node
        for node in ast.walk(register_api.body[247])
        if isinstance(node, ast.Call)
...
def test_moved_handler_keeps_canonical_ast_and_all_unrelated_statement_parity() -> None:
    ...
    for index, before in enumerate(old_register.body):
        if index in {162, 163, 164, 165, 166, 253, 254, 255, 258, 259, 260, 261, 262, 263, 264, 265, 266}:
            continue
        if 167 <= index <= 178:
            continue
