...
def test_transport_has_exact_dependency_registration_and_composition_shape() -> None:
    ...
    assert register_api.body[251].value.func.id == (
        "register_character_feature_state_api_route"
    )
    assert isinstance(register_api.body[252], ast.Expr)
    assert register_api.body[252].value.func.id == "register_character_currency_api_route"
    assert isinstance(register_api.body[253], ast.Expr)
    assert register_api.body[253].value.func.id == "register_character_notes_api_route"
    assert isinstance(register_api.body[254], ast.Expr)
    assert register_api.body[254].value.func.id == "register_character_personal_api_route"
    assert not any(
...
def test_moved_handler_keeps_canonical_ast_and_all_unrelated_statement_parity() -> None:
    ...
    for index, before in enumerate(old_register.body):
        if index in {162, 163, 164, 165, 166, 253, 254, 255, 264, 265, 266}:
            continue
        if 167 <= index <= 178:
            continue
