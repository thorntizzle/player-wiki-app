...
function Remove-VerifiedShortRootValidationWorktree {
    ...
    $resolvedDestination = Resolve-ValidationPath $Destination
    if (-not (Test-ValidationPathWithin -Path $resolvedDestination -Parent $Base)) {
        throw "Refusing cleanup outside the selected short-root base."
    }
    if ((Split-Path $resolvedDestination -Leaf) -ne $GeneratedLeaf -or $GeneratedLeaf -notmatch '^cpw-[0-9a-f]{7}-[0-9]+-[0-9a-f]{8}$') {
    ...
    $gitOutput = & git -C $Snapshot.Root worktree remove --force $resolvedDestination 2>&1
    ...
    if ($gitExitCode -ne 0) {
        if (Test-RegisteredValidationWorktree -Source $Snapshot.Root -Destination $resolvedDestination) {
            throw "Git refused to deregister the verified generated short-root worktree."
        }
        if (Test-Path -LiteralPath $resolvedDestination) {
            Write-Host "Git deregistered the worktree but left generated validation files; removing only the verified generated path."
