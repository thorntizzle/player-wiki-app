...
- The local Slice 5.6b Session Character dialog adoption is covered in `tests/test_campaign_session_page.py`, `tests/test_character_read_shell_browser.py`, and `tests/test_static_assets.py` for structure, insertion and reinitialization paths, labels, keyboard/focus and Session-state preservation, native fallbacks, fail-safe gating, idempotence, loading exclusion, and Combat isolation. Its independently accepted runtime/test state is exact local commit `db6d0d7aac1eb81bc053b8fe8873843c76a43111`, tree `f7f09ec0ce22799df21ec916bdc3954f9e7393c3`, on `codex/flask-rewrite-phase5`; it is not on `main`, pushed, deployed, or live. No complete suite was run, and the assembled Phase 5 presentation-domain freeze remains the promotion gate.
