...
- `systems_api_routes.py` adds 13 rules for 12 Systems handlers to the existing API Blueprint rather than creating another Blueprint: six GET rules for five read handlers plus seven mutation handlers for source policy, entry overrides, custom-entry create/update/archive/restore, and campaign item-mechanics import. The landing and search paths keep the shared `api.systems_index` identifier; every other handler keeps its existing bare `api.*` identifier, including `api.systems_item_mechanics_import` and the four `api.systems_custom_entry_*` identifiers. Each method/path remains registered exactly once. The shared `/systems/sources` path continues to advertise GET, HEAD, OPTIONS, and PUT through automatic OPTIONS handling; the custom-entry and item-mechanics POST mutations retain implicit `OPTIONS` without `HEAD`.
