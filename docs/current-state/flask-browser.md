...
- `systems_api_routes.py` adds six GET rules for five Systems read handlers to the existing API Blueprint rather than creating another Blueprint. The two landing and search paths keep the shared `api.systems_index` identifier, and the four source/category/entry handlers keep their existing `api.*` identifiers. The shared `/systems/sources` path continues to register GET and PUT exactly once each and advertises GET, HEAD, OPTIONS, and PUT through automatic OPTIONS handling.
