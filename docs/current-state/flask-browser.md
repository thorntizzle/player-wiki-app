...
- `systems_api_routes.py` adds eight rules for seven Systems handlers to the existing API Blueprint rather than creating another Blueprint: six GET rules for five read handlers plus the source-policy and entry-override PUT handlers. The landing and search paths keep the shared `api.systems_index` identifier; the other handlers keep their existing `api.*` identifiers, including `api.systems_source_update` and `api.systems_entry_override_update`. The shared `/systems/sources` path continues to register GET and PUT exactly once each and advertises GET, HEAD, OPTIONS, and PUT through automatic OPTIONS handling.
