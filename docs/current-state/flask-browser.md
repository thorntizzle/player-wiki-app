...
- `systems_api_routes.py` adds 12 rules for 11 Systems handlers to the existing API Blueprint rather than creating another Blueprint: six GET rules for five read handlers plus the source-policy and entry-override PUT handlers and four custom-entry create, update, archive, and restore handlers. The landing and search paths keep the shared `api.systems_index` identifier; every other handler keeps its existing bare `api.*` identifier, including `api.systems_custom_entry_create`, `api.systems_custom_entry_update`, `api.systems_custom_entry_archive`, and `api.systems_custom_entry_restore`. Each method/path remains registered exactly once. The shared `/systems/sources` path continues to advertise GET, HEAD, OPTIONS, and PUT through automatic OPTIONS handling; the four custom-entry mutations retain implicit `OPTIONS` without `HEAD`.
