...
- The TypeScript level-up parity path currently supports bounded TypeScript-created PHB Fighter, PHB Barbarian, PHB Rogue, and PHB Monk level-one to level-two saves; the Rogue slice adds Cunning Action, and the Monk slice adds Ki, Unarmored Movement, a 2-point Ki resource, the level-2 speed increase, HP/Hit Dice reconciliation, copied definition/import YAML writes, SQLite revision bumps, and native progression history.
