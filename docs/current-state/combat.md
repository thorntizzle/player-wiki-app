...
- Local Slice 5.6d makes selected-PC item and spell detail dialogs in player Combat,
  compatibility Combat Character, canonical DM Status, and compatibility `/combat/status` bounded
  adopters of the accepted shared presentation lifecycle. The shared controller owns the generic
...
- Slice 5.5 changes no Combat route, API, manager authorization, CSRF, service/store, persistence ordering, deletion policy, or source-record behavior. It is exact local commit `7311ba8c5b0774cb8e536c4ca43c658bcb2aec7b`, tree `38eb0aa83c05dc3d0d65269d360e430186778674`; it is not on `main`, pushed, deployed, or live.
- Slice 5.6d changes no query, hash, selected section, focus, draft, viewport, carousel, polling,
  ...
  runtime/test state is exact local commit `c0a442a275b8d7513a82f53cef9a8161cb8f67d8`, tree
...
- Local Slice 5.6d coverage in `tests/test_campaign_combat_page.py`,
  ...
  same 138 contract tests. No complete suite was run; promotion remains the assembled Phase 5
