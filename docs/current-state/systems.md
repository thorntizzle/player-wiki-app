...
- `player_wiki/systems_api_routes.py` owns JSON transport for 14 Systems handlers through 15 explicit registrations on the existing API Blueprint: seven read handlers across eight GET rules plus the seven source-policy, entry-override, custom-entry create/update/archive/restore, and campaign item-mechanics import mutation handlers. Landing and search share `api.systems_index`; all other handlers retain their existing bare `api.*` identifiers, including `api.systems_import_run_list`, `api.systems_import_run_detail`, and `api.systems_item_mechanics_import`. The import-run list and detail transports remain app-admin-only, read-only GETs with implicit `HEAD` and `OPTIONS`; they expose stored import history through the existing serializer, including raw `source_path` and full summaries. `api.py` retains the Blueprint, shared decorators, request and error helpers, import-run serializer and store composition, full DM Content Systems-payload builder, repository/service composition, dependency wiring, app-admin DND-5E ingest, and all other nonmoved Systems API routes. The source-list GET and source-policy PUT remain one rule per method on the same path, whose OPTIONS response advertises GET, HEAD, OPTIONS, and PUT; each custom-entry and item-mechanics POST mutation remains one rule with implicit `OPTIONS` and no `HEAD`.
