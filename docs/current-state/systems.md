...
- `player_wiki/systems_api_routes.py` owns JSON transport for 11 Systems handlers through 12 explicit registrations on the existing API Blueprint: five read handlers across six GET rules plus the source-policy and entry-override PUT handlers and the four custom-entry create, update, archive, and restore handlers. Landing and search share `api.systems_index`; all other handlers retain their existing bare `api.*` identifiers. `api.py` retains the Blueprint, shared decorators, request and error helpers, serializers, full DM Content Systems-payload builder, repository/service composition, dependency wiring, and all nonmoved Systems API routes, including campaign item mechanics import and admin ingest. The source-list GET and source-policy PUT remain one rule per method on the same path, whose OPTIONS response advertises GET, HEAD, OPTIONS, and PUT; each custom-entry mutation remains one rule with implicit `OPTIONS` and no `HEAD`.
