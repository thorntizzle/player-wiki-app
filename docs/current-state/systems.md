...
- `player_wiki/systems_api_routes.py` owns JSON transport for seven Systems handlers through eight explicit registrations on the existing API Blueprint: five read handlers across six GET rules plus the source-policy and entry-override PUT handlers. Landing and search share `api.systems_index`; all other handlers retain their existing `api.*` identifiers, including `api.systems_source_update` and `api.systems_entry_override_update`. `api.py` retains the Blueprint, shared decorators, request and error helpers, serializers, repository/service composition, dependency wiring, and all nonmoved Systems API mutations/imports. The source-list GET and source-policy PUT remain one rule per method on the same path, whose OPTIONS response advertises GET, HEAD, OPTIONS, and PUT.
