...
- The app-admin JSON ingest uses the same archive and importer boundaries but preserves its existing API-specific response and audit contract. It returns serialized import results and refetched import runs, writes no auth audit event, and can therefore leave completed import runs and replaced entries durable if run refetch or response serialization later fails.
