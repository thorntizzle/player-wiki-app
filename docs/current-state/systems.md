...
- `player_wiki/systems_api_routes.py` owns JSON transport for 12 Systems handlers through 13 explicit registrations on the existing API Blueprint: five read handlers across six GET rules plus the seven source-policy, entry-override, custom-entry create/update/archive/restore, and campaign item-mechanics import mutation handlers. Landing and search share `api.systems_index`; all other handlers retain their existing bare `api.*` identifiers, including `api.systems_item_mechanics_import`. `api.py` retains the Blueprint, shared decorators, request and error helpers, serializers, full DM Content Systems-payload builder, repository/service composition, dependency wiring, and all nonmoved Systems API routes, including app-admin DND-5E ingest and import-run reads. The source-list GET and source-policy PUT remain one rule per method on the same path, whose OPTIONS response advertises GET, HEAD, OPTIONS, and PUT; each custom-entry and item-mechanics POST mutation remains one rule with implicit `OPTIONS` and no `HEAD`.
