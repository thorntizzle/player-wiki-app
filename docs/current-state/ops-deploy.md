...
- The request trail in `player_wiki/app.py` (roughly lines 1804–2090) records bounded request start, slow-response, and exception details when `PLAYER_WIKI_REQUEST_TRAIL_ENABLED` is explicitly enabled. `player_wiki/config.py` defaults `REQUEST_TRAIL_ENABLED` to false. The trail omits health/static requests; the candidate gate does not enable it.
