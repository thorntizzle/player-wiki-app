...
- The TypeScript API gate runs `npm ci`, `scripts/route_snapshots.py --check`, `npm --prefix apps/api run typecheck`, `npm --prefix apps/api run build`, `npm --prefix apps/api run test:sqlite-startup-posture`, and `npm --prefix apps/api run test:route-parity`. Use `-SkipTsApiInstall` only when `apps/api/node_modules` is already current, and `-SkipRouteSnapshotCheck` only for an npm-only diagnostic.
