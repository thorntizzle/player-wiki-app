...
- Run `local.ps1 -Action contract` for a fast route, API, access-policy, and representative read-boundary check. Use `local.ps1 -Action test-focused -TestPath ...` for an explicit domain selection, `local.ps1 -Action test-restore` for the maintained recovery lane, `local.ps1 -Action test-browser` for the maintained browser/static lane, `local.ps1 -Action test-serial` for shared-resource-sensitive coverage, and `local.ps1 -Action test` for the complete regression suite at its policy or milestone gates. Add `-PhysicalShortRoot` when decisive Windows evidence requires a physical short checkout rather than only short `.local` temp paths.
