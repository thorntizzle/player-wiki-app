...
- Keep browser-owned global-search behavior in its inline controller: search and preview fetches, cancellation and debounce, status/results/error rendering, preview insertion, and live and busy updates. Keep authorization and access filtering in the Flask routes, and keep `safe_rich_html` preview sanitization and the real dedicated-page link in the server/template pipeline. Generic dialog mechanics belong to the shared presentation controller, and in-page preview must not reveal the global loading cover.
