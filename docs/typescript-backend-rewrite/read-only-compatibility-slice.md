...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character Cultivation context and `save_insight` / `record_gathering_insight` / `spend_cultivation_energy` / `spend_meditation_yin_yang` / `spend_conditioning` / `spend_training` / `advance_martial_art_rank` write slices, including manager-only access, missing-character JSON, supported Xianxia read context, Generic Technique option filtering, Realm Ascension read state, nested and flat JSON save payloads, stale revision conflicts, non-negative Insight validation, Gathering Insight positive whole-number validation, Cultivation Energy key and available-Insight validation, Meditation Yin/Yang key and available-Insight validation, Conditioning HP/Effort target validation, Training Stance/Attribute target validation, Martial Art selection/rank/structured metadata/Insight validation, available-Insight validation, HP and Stance maximum cap validation, definition YAML Insight/Energy/Yin-Yang/Conditioning/Training/Martial Art/history persistence, state revision bumps without refilling current HP/Stance/Energy/Yin/Yang pools, unchanged-save behavior without duplicate history, and unsupported non-Xianxia POST validation.
