...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character Cultivation context and `save_insight` / `record_gathering_insight` / `spend_cultivation_energy` write slices, including manager-only access, missing-character JSON, supported Xianxia read context, Generic Technique option filtering, Realm Ascension read state, nested and flat JSON save payloads, stale revision conflicts, non-negative Insight validation, Gathering Insight positive whole-number validation, Cultivation Energy key and available-Insight validation, definition YAML Insight/Energy/history persistence, state revision bumps without refilling current Energy pools, unchanged-save behavior without duplicate history, and unsupported non-Xianxia POST validation.
