...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character Cultivation context and `save_insight` / `record_gathering_insight` write slices, including manager-only access, missing-character JSON, supported Xianxia read context, Generic Technique option filtering, Realm Ascension read state, nested and flat JSON save payloads, stale revision conflicts, non-negative Insight validation, Gathering Insight positive whole-number validation, definition YAML Insight/history persistence, state revision bumps, unchanged-save behavior without duplicate history, and unsupported non-Xianxia POST validation.
