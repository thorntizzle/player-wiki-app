...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character Cultivation context and `save_insight` write slice, including manager-only access, missing-character JSON, supported Xianxia read context, Generic Technique option filtering, Realm Ascension read state, nested and flat JSON save payloads, stale revision conflicts, non-negative Insight validation, definition YAML Insight/history persistence, state revision bumps, unchanged-save behavior without duplicate history, and unsupported non-Xianxia POST validation.
