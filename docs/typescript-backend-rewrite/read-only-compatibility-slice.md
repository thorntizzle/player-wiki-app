...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character Advanced Editor context read route, including unauthenticated auth_required behavior, DM and assigned-player DND-5E success payloads, unassigned-player forbidden behavior, missing-character JSON, unsupported non-DND payloads, edit-page links, state revision, proficiency, stat-adjustment, recoverable-penalty, and reference field names, Flask-shaped custom-feature row indexes/values/options, and Flask-shaped manual-equipment row indexes/values.
  - validates the Character Advanced Editor reference/proficiency/stat-adjustment/recoverable-penalty/custom-feature/manual-equipment-field write route, including fixture-role write denial, unsupported field validation for fields outside this slice, linked custom-feature page validation, stale revision conflict, copied-fixture `definition.yaml` profile/reference-note/proficiency/stat-adjustment/recoverable-penalty/custom-feature/manual-equipment writes, managed `import.yaml` writes, SQLite notes persistence for state-backed fields, current-HP clamping when max HP is lowered, SQLite resource reconciliation for unlinked custom-feature tracker add/update/remove, SQLite inventory reconciliation for unlinked manual equipment add/update/remove, refreshed reference/proficiency/stat-adjustment/recoverable-penalty/custom-feature/manual-equipment field payload values, page-linked manual equipment validation, and revision bump behavior. Linked custom-feature page/choice/spell derivation, page-linked manual item option derivation, full native edit derivation, and complete Advanced Editor save parity remain pending.
