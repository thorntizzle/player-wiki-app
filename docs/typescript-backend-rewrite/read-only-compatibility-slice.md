...
- Character create context payload checks cover:
  ...
  - DM/admin success for the GET-only DND-5E Gen2 payload shape, with POST submit intentionally outside this slice.
...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character create context read route, including unauthenticated auth_required behavior, player forbidden behavior, DM/admin DND-5E context shape, missing-campaign JSON, unsupported-system JSON, and Xianxia create field/default payloads after switching the copied fixture campaign to Xianxia.
