...
- Added the Character Advanced Editor reference/proficiency/stat-adjustment/recoverable-penalty/custom-feature/manual-equipment-field write endpoint:
  - `PUT /api/v1/campaigns/:campaignSlug/characters/:characterSlug/advanced-editor`
- Added the Character advancement context endpoints:
...
- Added the Character advancement POST endpoints:
  - `POST /api/v1/campaigns/:campaignSlug/characters/:characterSlug/retraining`
  - `POST /api/v1/campaigns/:campaignSlug/characters/:characterSlug/level-up`
  - `POST /api/v1/campaigns/:campaignSlug/characters/:characterSlug/progression-repair`
  - The retraining pair now includes the first DND-5E advancement save-parity slice: empty-readiness shells stay friendly for characters without supported options, while ready linked custom-feature choices hydrate from the Advanced Editor context and save through the shared copied-fixture definition/import YAML plus SQLite state-revision path. Level-up now has a bounded TypeScript-created DND-5E Fighter/Barbarian level-one to level-two context/save slice with copied-fixture definition/import YAML writes, SQLite state revision bumps, HP/Hit Dice/resource reconciliation, and `native_progression` level-up history. Progression repair now has a bounded imported DND-5E class/subclass/species/background ref context/save slice with copied-fixture definition/import YAML writes, SQLite revision bumps, `native_progression` baseline repair/history evidence, and Level Up handoff payloads once the bounded ref repair is complete.
- Added the Character Cultivation context shell endpoint:
  ...
  - This TypeScript slice verifies the manager-gated DND-5E unsupported shell against current fixtures, returns a supported Xianxia read context with Insight/resources, Martial Art advancement, Generic Technique options, Realm Ascension read state, history rows, and links, and saves the narrow `save_insight`, `record_gathering_insight`, `spend_cultivation_energy`, `spend_meditation_yin_yang`, `spend_conditioning`, `spend_training`, `advance_martial_art_rank`, `learn_generic_technique`, `start_realm_ascension_review`, `reset_realm_ascension_stats`, `apply_immortal_realm_rebuild`, `apply_divine_realm_rebuild`, and `confirm_realm_ascension` mutations with stale conflict, validation, definition YAML, state revision, and history-row coverage.
...
- Character create context payload checks cover:
  ...
  - DND-5E POST submit for the bounded PHB Fighter or PHB Barbarian / Human / Soldier level-one slice, including server-side Systems selection validation, `builder://dnd5e-create-level-one` definition/import metadata, initialized SQLite state, duplicate-slug `character_exists`, and validation errors for out-of-slice DND selections. Full DND level-one builder write parity remains pending.
...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character create route pair, including unauthenticated auth_required behavior, player forbidden behavior, DM/admin DND-5E builder-ready context shape, DND-5E bounded level-one submit success/duplicate/out-of-slice validation, missing-campaign JSON, unsupported-system JSON, Xianxia create field/default payloads, and Xianxia native create writes with initialized SQLite state and duplicate-slug `character_exists` after switching the copied fixture campaign to Xianxia.
  ...
  - validates the Character Advanced Editor context read route, including unauthenticated auth_required behavior, DM and assigned-player DND-5E success payloads, unassigned-player forbidden behavior, missing-character JSON, unsupported non-DND payloads, edit-page links, state revision, proficiency, stat-adjustment, recoverable-penalty, and reference field names, Flask-shaped custom-feature row indexes/values/options, Flask-shaped manual-equipment row indexes/values, and hydrated linked campaign-page selector options.
- validates the Character Advanced Editor reference/proficiency/stat-adjustment/recoverable-penalty/custom-feature/manual-equipment-field write route, including fixture-role write denial, unsupported field validation for fields outside this slice, stale revision conflict, copied-fixture `definition.yaml` profile/reference-note/proficiency/stat-adjustment/recoverable-penalty/custom-feature/manual-equipment writes with custom-feature and manual-equipment `page_ref` validation/round-trip, managed `import.yaml` writes, SQLite notes persistence for state-backed fields, current-HP clamping when max HP is lowered, SQLite resource reconciliation for custom-feature tracker add/update/remove, SQLite inventory reconciliation for manual equipment add/update/remove, refreshed reference/proficiency/stat-adjustment/recoverable-penalty/custom-feature/manual-equipment field payload values, and revision bump behavior. Linked custom-feature page/choice/spell derivation and full native edit derivation remain pending. Manual item option derivation from linked manual page metadata is implemented.
  - validates the Character advancement context routes, including unauthenticated auth_required behavior, DM and assigned-owner shell reads, route-specific forbidden behavior, missing-character JSON, current fixture unsupported/empty readiness payloads, Advanced Editor/detail links, and current unsupported `repair` contexts. It also validates matching unsupported POST shells for current DND and Xianxia fixture states, including Flask-compatible `unsupported_campaign_system` messages. The retraining golden proof now covers ready-state linked custom-feature context hydration and bounded save parity: invalid choice validation, copied-fixture definition/import writes, SQLite state revision bumping, generated Optional Feature replacement, and `native_progression` retrain history. The DND create/level-up proof covers the TypeScript-created Fighter level-one to level-two slice: ready context hydration, HP Gain validation, copied-fixture definition/import writes, SQLite state revision bumping, HP/Hit Dice/resource reconciliation, and `native_progression` `hp_baseline` plus `level_up` history. The progression-repair proof now covers imported DND-5E class/subclass/species/background ref context hydration, stale state conflict behavior, copied-fixture definition/import writes, SQLite state revision bumping, `native_progression` baseline repair/history evidence, and Level Up handoff payloads once the bounded ref repair is complete. Broader legacy spell-mark, feat, optional-feature, and full imported-derivation repair parity remain pending.
  - validates the Character Cultivation context and `save_insight` / `record_gathering_insight` / `spend_cultivation_energy` / `spend_meditation_yin_yang` / `spend_conditioning` / `spend_training` / `advance_martial_art_rank` / `learn_generic_technique` / `start_realm_ascension_review` / `reset_realm_ascension_stats` / `apply_immortal_realm_rebuild` / `apply_divine_realm_rebuild` / `confirm_realm_ascension` write slices, including manager-only access, missing-character JSON, supported Xianxia read context, Generic Technique option filtering, Realm Ascension read state, nested and flat JSON save payloads, stale revision conflicts, non-negative Insight validation, Gathering Insight positive whole-number validation, Cultivation Energy key and available-Insight validation, Meditation Yin/Yang key and available-Insight validation, Conditioning HP/Effort target validation, Training Stance/Attribute target validation, Martial Art selection/rank/structured metadata/Insight validation, Generic Technique selection/catalog/direct-spend/duplicate/Insight validation, Realm Ascension target/prerequisite/review-note/reset/rebuild/final-confirmation sequencing validation, HP and Stance maximum cap validation, definition YAML Insight/Energy/Yin-Yang/Conditioning/Training/Martial Art/Generic Technique/Realm Ascension/history persistence, state revision bumps without refilling current HP/Stance/Energy/Yin/Yang pools, unchanged-save behavior without duplicate history, and unsupported non-Xianxia POST validation.
