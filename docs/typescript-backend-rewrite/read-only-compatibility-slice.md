...
- Added the Character Advanced Editor reference-field write endpoint:
...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character Advanced Editor context read route, including unauthenticated auth_required behavior, DM and assigned-player DND-5E success payloads, unassigned-player forbidden behavior, missing-character JSON, unsupported non-DND payloads, edit-page links, state revision, reference field names, and non-empty feature/equipment row shells.
  - validates the Character Advanced Editor reference-field write route, including fixture-role write denial, unsupported field validation for fields outside this slice, stale revision conflict, copied-fixture `definition.yaml` profile/reference-note writes, SQLite notes persistence for state-backed fields, refreshed reference-field payload values, and revision bump behavior. Feature/equipment rows, full native edit derivation, broader state merging, and complete Advanced Editor save parity remain pending.
