...
- `apps/api/tests/smoke.mjs`:
  ...
  - validates the Character Advanced Editor context read route, including unauthenticated auth_required behavior, DM and assigned-player DND-5E success payloads, unassigned-player forbidden behavior, missing-character JSON, unsupported non-DND payloads, edit-page links, state revision, proficiency, stat-adjustment, recoverable-penalty, and reference field names, Flask-shaped manual-equipment row indexes/values, and non-empty feature row shells.
  - validates the Character Advanced Editor reference/proficiency/stat-adjustment/recoverable-penalty/manual-equipment-field write route, including fixture-role write denial, unsupported field validation for fields outside this slice, custom-feature-row rejection, stale revision conflict, copied-fixture `definition.yaml` profile/reference-note/proficiency/stat-adjustment/recoverable-penalty/manual-equipment writes, SQLite notes persistence for state-backed fields, current-HP clamping when max HP is lowered, SQLite inventory reconciliation for unlinked manual equipment add/update/remove, refreshed reference/proficiency/stat-adjustment/recoverable-penalty/manual-equipment field payload values, page-linked manual equipment validation, and revision bump behavior. Custom feature rows, page-linked manual item option derivation, full native edit derivation, `import.yaml` writes, and complete Advanced Editor save parity remain pending.
