...
- `apps/api` exists and now serves:
  ...
  - `POST /api/v1/campaigns/:campaignSlug/characters/:characterSlug/cultivation` (`save_insight`, `record_gathering_insight`, `spend_cultivation_energy`, `spend_meditation_yin_yang`, `spend_conditioning`, `spend_training`, and `advance_martial_art_rank` only; remaining action parity pending)
...
- The first Character Cultivation write slices now serve
  ...
  `spend_training`, and `advance_martial_art_rank` actions. They preserve
  ...
  `training_attribute_increase`, and `martial_art_rank_advance` history rows, and refreshed
