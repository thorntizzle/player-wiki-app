...
- `apps/api` exists and now serves:
  ...
  - `POST /api/v1/campaigns/:campaignSlug/characters/create` (Xianxia native create; bounded DND-5E PHB Fighter/Barbarian level-one slice plus PHB Bard known-spell, PHB Cleric/Life Domain, PHB Druid prepared-spell, PHB Rogue expertise/equipment, PHB Ranger feature/equipment, PHB Monk Unarmored Defense/Martial Arts, PHB Paladin Divine Sense/Lay on Hands, PHB Sorcerer/Draconic Bloodline known-spell, PHB Warlock/Fiend Pact Magic, and PHB Wizard spellcasting create slices; full DND builder parity pending)
  ...
  - `GET /api/v1/campaigns/:campaignSlug/characters/:characterSlug/level-up` (unsupported shells plus bounded ready context for TypeScript-created DND-5E Fighter/Barbarian/Rogue/Monk level-one sheets)
  - `POST /api/v1/campaigns/:campaignSlug/characters/:characterSlug/level-up` (bounded TypeScript-created DND-5E Fighter/Barbarian/Rogue/Monk level-one to level-two fixture save; broader save parity pending)
...
- The Character create route pair now serves `GET .../characters/create` plus Xianxia-lane
  ...
  writes a bounded DND-5E PHB Fighter, PHB Barbarian, PHB Bard, PHB Cleric, PHB Druid, PHB Rogue, PHB Ranger, PHB Monk, PHB Paladin, PHB Sorcerer, PHB Warlock, or PHB Wizard level-one slice with enabled Systems species/background rows from the current supported source matrix through the same persistence path with
  ...
  The Rogue slice writes Expertise, Sneak Attack, Thieves' Cant, bounded proficiencies/skills/tool expertise, and a leather/rapier/shortbow/dagger/thieves' tools starter equipment package.
  The Ranger slice writes Favored Enemy and Natural Explorer feature rows, bounded proficiencies/skills, no level-one spellcasting, and a leather/two-shortsword/longbow starter equipment package.
  The Monk slice writes Unarmored Defense and Martial Arts feature rows, bounded Acrobatics/Insight skills, deterministic Calligrapher's supplies proficiency, no level-one spellcasting or Ki, and a shortsword/darts/explorer's pack starter equipment package.
  The Paladin slice writes Divine Sense and Lay on Hands tracked resources, bounded proficiencies/skills, no level-one spellcasting, and a chain-mail/shield/longsword/javelin starter equipment package.
  The Sorcerer slice requires the PHB Draconic Bloodline origin, writes selected cantrips/known spells, Sorcerous Origin/Draconic Ancestry/Draconic Resilience feature rows, Charisma spell DC/attack, first-level spell slots, Draconic language, Draconic Resilience HP/AC, bounded proficiencies/skills, and a crossbow/dagger/component-pouch starter package.
  The Warlock slice requires the PHB Fiend patron, writes selected cantrips/known spells, Otherworldly Patron/Pact Magic/Dark One's Blessing feature rows, Charisma spell DC/attack, bounded proficiencies/skills, a leather/crossbow/dagger starter package, and a distinct first-level Pact Magic slot lane in definition and SQLite state.
...
- The Character retraining route now serves the first DND-5E advancement save-parity slice. `GET .../characters/:characterSlug/retraining`
  ...
  hydrates a ready level-1 to level-2 context for the supported Fighter/Barbarian/Rogue/Monk slice, while
  ...
  `source.native_progression.hp_baseline` plus `level_up` history, appends Rogue `Cunning Action`, appends Monk `Ki` and `Unarmored Movement` with a 2-point Ki resource and the level-2 speed increase, and returns refreshed level-up payloads.
