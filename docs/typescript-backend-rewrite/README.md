...
- `apps/api` exists and now serves:
  ...
  - `POST /api/v1/campaigns/:campaignSlug/characters/:characterSlug/cultivation` (`save_insight` and `record_gathering_insight` only; remaining action parity pending)
...
- The first Character Cultivation write slices now serve
  `POST .../characters/:characterSlug/cultivation` for the `save_insight` and `record_gathering_insight` actions. They preserve
  ...
  positive whole-number Gathering Insight validation,
  copied-fixture `definition.yaml` writes, SQLite state revision bumps and Xianxia state
  reconciliation, `insight_counter_adjustment` and `gathering_insight` history rows, and
  refreshed Cultivation payloads. Remaining Cultivation action parity remains pending.
