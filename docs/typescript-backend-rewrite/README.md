...
- `apps/api` exists and now serves:
  ...
  - `POST /api/v1/campaigns/:campaignSlug/characters/:characterSlug/cultivation` (`save_insight`, `record_gathering_insight`, and `spend_cultivation_energy` only; remaining action parity pending)
...
- The first Character Cultivation write slices now serve
  ...
  and `spend_cultivation_energy` actions. They preserve
  ...
  `cultivation_energy_increase` history rows, and refreshed Cultivation payloads. Remaining
  Cultivation action parity remains pending.
