...
- `apps/api` exists and now serves:
  ...
  - `GET /api/v1/campaigns/:campaignSlug/characters/:characterSlug/level-up` (unsupported shells plus bounded ready context for TypeScript-created DND-5E Fighter/Barbarian/Rogue level-one sheets)
  - `POST /api/v1/campaigns/:campaignSlug/characters/:characterSlug/level-up` (bounded TypeScript-created DND-5E Fighter/Barbarian/Rogue level-one to level-two fixture save; broader save parity pending)
...
- The Character retraining route now serves the first DND-5E advancement save-parity slice. `GET .../characters/:characterSlug/retraining`
  ...
  hydrates a ready level-1 to level-2 context for the supported Fighter/Barbarian/Rogue slice, while
  ...
  `source.native_progression.hp_baseline` plus `level_up` history, appends Rogue `Cunning Action` for the bounded Rogue slice, and returns refreshed level-up payloads.
