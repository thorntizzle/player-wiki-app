...
- `apps/api` exists and now serves:
  ...
  - `PUT /api/v1/campaigns/:campaignSlug/characters/:characterSlug/advanced-editor` (reference and proficiency fields only; feature/equipment/native derivation parity pending)
...
- The Character Advanced Editor context route now serves `GET .../characters/:characterSlug/advanced-editor`,
  ...
  reference fields, proficiency fields, and feature/equipment row shells. The `PUT .../advanced-editor`
  reference/proficiency-field slice now serves bearer-token writes for the existing reference and
  proficiency fields, preserving fixture-role write denial, unsupported non-DND validation, stale
  revision conflicts, copied-fixture `definition.yaml` writes for profile/reference notes/proficiencies,
  SQLite notes persistence for state-backed reference fields, and refreshed Advanced Editor payloads.
  Feature/equipment rows, full native edit derivation, broader state merging, and complete Advanced
  Editor parity remain pending.
