...
- The first character session-state write routes now serve
  ...
  `PATCH .../characters/:characterSlug/session/resources/:resourceId` for bearer API-token app
  ...
  updates, and Xianxia HP/temp HP plus Stance/Energy/Yin-Yang/Dao fields, deny fixture-role writes,
  and return refreshed `character.state_record` payloads. Current validation covers disposable
  copied fixture files and fixture SQLite only; the remaining character session-state route family is
  still sliced separately.
