...
| Character advancement parity | Route parity | Retraining now has the first bounded DND-5E save-parity slice: ready-state linked custom-feature context hydration, structured-choice validation, copied-fixture definition/import YAML writes, SQLite state revision bumps, and `native_progression` retrain history are covered by focused Flask-vs-TypeScript golden proof. Level-up and progression-repair context shells and unsupported/current-shell POST responses still exist. | Level-up and progression-repair real save parity remain pending, along with broader retraining beyond linked custom-feature structured choices. | Golden tests for level-up and progression repair on ready and repairable DND sheets, plus any widened retraining cases; fixture SQLite/file assertions and Gen2 smoke coverage for each new advancement family. |
