...
| Combat writes | Data migration | Combat tracker reads/writes, combatant CRUD, vitals/resources, NPC resources, conditions, and turn flow have disposable DB validation. `combat-rehearsal-readiness.md` and the staging harness now provide a Combat-specific copied-data transcript guide with tested table-capture/drift evidence. | Needs an actual copied-data rehearsal with player-character mirrors, source-backed NPC resources, revisions, backup, mutation, restore, and equivalence before the family can claim `copied-data rollback ready`. | Completed copied-data transcript covering player Combat, DM Status, DM Controls, selected-PC state writes, revisions, source-backed resources, conditions, clear/delete behavior, and restore equivalence. |
