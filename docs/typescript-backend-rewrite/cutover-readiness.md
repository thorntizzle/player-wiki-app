...
| Local Windows ops | Ops/deploy | Current contract uses `local.ps1`, workspace venv or bundled/runtime-specific tooling, and `.local/tmp` scratch boundaries; `local.ps1 -Action ts-api-check` now resolves Node/npm without global `npm` and runs install, route snapshot, typecheck, build, and route-parity checks. | The API typecheck/build/route-parity wrapper gap is closed for local Windows validation. Local compiled-start smoke and Docker/Fly packaging proof remain separate gates. | Keep the wrapper green on integration branches, and continue collecting local compiled-start, container, `/healthz`, `/app-next/`, asset, and rollback evidence before deploy or cutover approval. |
| Docker/Fly packaging | Ops/deploy | Flask Fly shape is one app, one `/data` volume, SQLite at `/data/player_wiki.sqlite3`, campaign content at `/data/campaigns`, and `/healthz`. The Dockerfile now has a non-default `ts-api-runtime-proof` target and static packaging proof for TypeScript API build/start/env mapping. The 2026-06-28 no-deploy pass also proved the compiled TypeScript API can start locally from `apps/api/dist/server.js` against copied fixture data and answer `/healthz` plus `/api/v1/app` with the expected disposable path metadata. Hono stack spike did not run Fly deploy. | TypeScript local image build, container startup command execution, schema initialization, frontend build integration, and sanitized `fly.toml` behavior still need runtime packaging proof. Docker was unavailable for the current no-deploy image build. | Local production build plus container smoke, `/healthz`, `/app-next/` if frontend is bundled, representative asset content type, and no tracked private Fly identifiers. |
