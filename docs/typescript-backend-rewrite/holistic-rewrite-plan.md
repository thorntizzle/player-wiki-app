...
two angles. The backend needs to build V2 domain foundations rather than
preserve Flask implementation shapes. The frontend needs stable, typed,
responsive app contracts so Gen2 can become a richer tabletop workspace instead
of a page-for-page compatibility shell.

This plan combines:

- `docs/typescript-backend-rewrite/architecture-opportunity-audit.md`
- `docs/typescript-backend-rewrite/v2-rewrite-strategy.md`
- `docs/typescript-backend-rewrite/legacy-data-migration-plan.md`
...
Current-state docs remain evidence for current data, permissions, workflows, and
migration risk. This plan describes the path from Flask-era data and workflows
to a cleaner V2 TypeScript backend and a calmer, denser, more capable Gen2
workspace.

## Operating Boundary

- Flask remains production authority until the V2 cutover gates pass.
...
- Direct Flask routes and legacy browser JSON routes are compatibility paths for
  migration, cutover, or rollback only unless a later plan explicitly promotes
  them.
...
Campaign Player Wiki should become a stable V2 TypeScript-backed tabletop
...
- API routes are adapters over canonical V2 service functions, typed payloads,
  and explicit error semantics. `/api/v1` is a compatibility edge unless a later
...
- Legacy data migration, rollback, schema, asset, and fallback decisions stay
  visible until they are intentionally closed.

## Planning Principles

| Principle | Backend meaning | Frontend meaning |
| --- | --- | --- |
| Preserve data and safety contracts, not incidental Flask structure. | Auth, visibility, revisions, rollback, migrated data integrity, and critical workflow semantics stay strict; route handlers become adapters over V2 services. | Existing Gen2 routes and fallback links stay stable during transition while internal data hooks and workspace layouts improve. |
...
| Treat compatibility as an edge concern. | Browser JSON and Flask-shaped `/api/v1` routes are shims for migration/cutover/rollback where needed. | Gen2 can keep old endpoints during transition, then migrate to typed query/mutation hooks after the V2 contract settles. |
...
| --- | --- | --- | --- |
| API and service boundary | Keep `/api/v1` and compatibility routes thin while centralizing V2 domain behavior. | V2 strategy, current-state docs, migration inventory, error-shape decision. | Route adapters, service modules, typed DTOs, explicit compatibility exceptions. |
...
| Legacy data migration | Non-destructively import current SQLite, YAML, Markdown, and asset data into V2 structures. | V2 schema decision, domain models, current data inventory, copied/staging snapshots. | Import adapters, mapping manifests, validation reports, dry-run output, rollback/export plan. |
| Ops, schema, and cutover | Keep migration, packaging, rollback, and staging evidence honest. | SQLite/V2 schema ownership decision, container/frontend hosting decision, staging rehearsals. | V2 readiness labels, schema posture, packaging proof, rollback transcript. |

## Phase Plan

### Phase 0: Integration Discipline

Goal: prevent current parity work from becoming accidental architecture and
reset progress tracking around V2 readiness.
...
- Stop treating "implemented route" as "architecturally final" or as the main
  progress metric.
- Track V2 domain coverage and legacy data migration coverage alongside any
  route compatibility work.
- Keep Gen2 behavior stable; do not start broad frontend redesign while backend
  contracts are moving.

Exit criteria:

- New cutover-readiness updates include architecture classification and V2 data
  migration posture.
...
- What are the canonical V2 domain models and storage boundaries for characters,
  content, Systems, Session, Combat, DM Content, and assets?
- What is the canonical DND progression data source: Systems metadata, curated
  TypeScript tables, imported content, or hybrid?
- Does V2 use a new schema/import target, evolve the current SQLite schema, or
  run a hybrid during migration?
...
- Decision docs or ADRs exist for V2 domain/storage shape, DND progression,
  schema/import ownership, image/portrait policy, API compatibility, and
  frontend hosting.
...
Goal: build the reusable V2 backend surface and legacy-data bridge that Gen2
workspaces can trust.
...
- Normalize Systems metadata enough to drive DND class/subclass/spell/feat/item
  choices.
- Define non-destructive import adapters and validation reports for legacy
  characters, content, Systems, Session, Combat, DM Content, and assets.
...
- At least one existing parity treadmill is replaced by a reusable V2 domain
  module.
- Current golden tests still pass where they protect migration, data integrity,
  security, or user-critical behavior.
- Gen2-facing payloads for the chosen pilot are stable and documented.

### Phase 3: Cutover Parity And Rehearsal

Goal: make V2 safe enough to replace Flask without coupling cutover to a large
frontend redesign.

- Preserve required `/api/v1` and browser JSON compatibility routes only where
  migration, current Gen2, cutover, or rollback needs them.
- Keep frontend changes limited to compatibility fixes and API hook alignment.
- Finish copied-data and staging-equivalent migration rehearsals for data
  families.
...
- Existing Gen2 workflows pass with migrated V2 data and TypeScript backend
  behavior.
