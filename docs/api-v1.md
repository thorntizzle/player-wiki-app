...
- `GET /api/v1/campaigns/<campaign_slug>/characters/create` returns the system-lane create context used by Gen2. DND-5E campaigns receive a builder-ready TypeScript context for the current level-1 option slice, including enabled Systems class/species/background/subclass options, sanitized selected values, ability-score fields, preview basics, spell-choice fields for the bounded Bard, Cleric, Druid, Sorcerer, Warlock, and Wizard slices, and current support-limit notes. Xianxia campaigns receive canonical Attribute/Effort/Energy/Skill fields, Martial Art/rank choices, GM grant fields, starting defaults, and import links. `POST .../characters/create` supports Xianxia native create and the narrow DND-5E PHB Fighter / PHB Barbarian / PHB Bard / PHB Cleric with Life Domain / PHB Druid / PHB Rogue / PHB Ranger / PHB Monk / PHB Paladin / PHB Sorcerer with Draconic Bloodline / PHB Warlock with Fiend patron / PHB Wizard level-one pilot lane with enabled Systems species/background rows from the current supported source matrix. The DND pilot rebuilds and validates the server-side Systems selections, writes `definition.yaml` and `import.yaml` with `builder://dnd5e-create-level-one` metadata, initializes SQLite mutable state, returns `character_exists` for duplicate slugs, and returns `validation_error` for out-of-pilot DND selections. The selected species/background titles and Systems refs are persisted in the definition; full background equipment/proficiency package derivation remains bounded. The Bard slice writes selected cantrips/known spells, Bardic Inspiration from Charisma modifier, and first-level spell slots. The Cleric slice writes selected cantrips/prepared spells, Life Domain always-prepared spells, and first-level spell slots. The Druid slice writes selected cantrips/prepared spells, Druidic and Spellcasting feature rows, first-level spell slots, bounded proficiencies/skills, and a leather/shield/scimitar starter equipment package. The Rogue slice writes Expertise, Sneak Attack, Thieves' Cant, bounded proficiencies/skills/tool expertise, and a leather/rapier/shortbow/dagger/thieves' tools starter equipment package. The Ranger slice writes Favored Enemy and Natural Explorer feature rows, bounded proficiencies/skills, no level-one spellcasting, and a leather/two-shortsword/longbow starter equipment package. The Monk slice writes Unarmored Defense and Martial Arts feature rows, bounded proficiencies/skills, no level-one spellcasting or Ki, and a shortsword/darts/explorer's pack starter equipment package. The Paladin slice writes Divine Sense and Lay on Hands tracked resources, bounded proficiencies/skills, no level-one spellcasting, and a chain-mail/shield/longsword/javelin starter equipment package. The Sorcerer slice requires PHB Draconic Bloodline, writes selected cantrips/known spells, Sorcerous Origin/Draconic Ancestry/Draconic Resilience feature rows, Charisma spell DC/attack, first-level spell slots, Draconic language, Draconic Resilience HP/AC, bounded proficiencies/skills, and a crossbow/dagger/component-pouch starter package. The Warlock slice requires PHB Fiend, writes selected cantrips/known spells, Otherworldly Patron/Pact Magic/Dark One's Blessing feature rows, a distinct first-level Pact Magic slot lane, Charisma spell DC/attack, bounded proficiencies/skills, and a leather/crossbow/dagger starter package. The Wizard slice writes selected cantrips, six selected spellbook spells, selected prepared spellbook spells, Arcane Recovery, first-level spell slots, and PHB Sage background refs when that enabled Systems row is selected. Full DND level-one builder write parity remains pending. The endpoint requires the same character-management access as Flask create.
...
- `GET .../advanced-editor` returns the DND-5E native edit field context, hydrated linked-page selector option lists, current state revision, links, and unsupported fallback metadata for non-DND lanes. The Flask/current API `PUT .../advanced-editor` accepts `expected_revision` plus a flat `values` object matching the returned field names, reuses the existing native edit apply/derivation path for proficiencies, reference text, stat adjustments, recoverable penalties, custom features, and manual equipment, then writes `definition.yaml`, `import.yaml`, and reconciled SQLite state. Advanced Editor writes are available to DMs/admins and assigned players with session-mode character access, return `409 state_conflict` on stale sheet revisions, and return `unsupported_campaign_system` for non-DND lanes. The TypeScript rewrite currently implements the reference-field, proficiency-field, stat-adjustment-field, recoverable-penalty-field, custom-feature, and manual-equipment save subset for `physical_description_markdown`, `background_markdown`, `biography_markdown`, `personality_markdown`, `additional_notes_markdown`, `allies_and_organizations_markdown`, `languages_text`, `armor_proficiencies_text`, `weapon_proficiencies_text`, `tool_proficiencies_text`, `stat_adjustment_max_hp`, `stat_adjustment_armor_class`, `stat_adjustment_initiative_bonus`, `stat_adjustment_speed`, `stat_adjustment_passive_perception`, `stat_adjustment_passive_insight`, `stat_adjustment_passive_investigation`, indexed `recoverable_penalty_*_<row>` fields, indexed `custom_feature_*_<row>` fields with linked-page selector hydration, `page_ref` validation, manual trackers, linked campaign feat defaults, `custom_feature_optionalfeature_<row>_<section>_<choice>` fields backed by campaign `optionalfeature_progression` plus enabled Systems `optionalfeature` rows, and linked campaign feat `custom_feature_additional_spells_<row>_<category>_<spec>_<choice>` fields backed by enabled Systems spell rows for additional-spell choice filters, indexed `manual_item_*_<row>` fields with linked-page selector hydration and `page_ref` validation, and managed `import.yaml` writes. Manual item linked-page option derivation, campaign-feat Optional Feature choices, and the narrow linked additional-spell choice slice are now implemented. Spell support/manager replacement derivation and full native edit derivation remain pending while Flask remains production authority.
...
- `GET .../level-up` returns the DND-5E native level-up context, current sheet revision, readiness metadata, dynamic choice sections, preview payload, Gen2/Flask links, and unsupported or repairable fallback metadata for non-ready lanes. `POST .../level-up` accepts `expected_revision` plus a flat `values` object matching the returned level-up field names, reuses the existing native level-up builder/apply path for one-level advancement and HP gain, then writes `definition.yaml`, `import.yaml`, and reconciled SQLite state. The TypeScript rewrite currently has a bounded TypeScript-created PHB Fighter, PHB Barbarian, PHB Rogue, PHB Ranger, PHB Monk, and PHB Paladin level-one to level-two save slice; Rogue adds Cunning Action, Ranger adds deferred-choice Fighting Style, Spellcasting, two first-level known-spell choices, and first-level Ranger spell slots, Monk adds Ki, Unarmored Movement, a 2-point Ki resource, and the level-2 speed increase, and Paladin adds deferred-choice Fighting Style, Spellcasting, Divine Smite, first-level Paladin spell slots, and Lay on Hands scaling with the same copied YAML, SQLite revision, HP/Hit Dice/resource reconciliation, and native progression history contract. Ranger/Paladin prepared-spell choice UI, broad Fighting Style UI, and broader spell growth remain pending. Level-up writes are manager-only, return `409 state_conflict` on stale sheet revisions, point repairable imported sheets at Gen2 progression repair with a Flask fallback link, and return `unsupported_campaign_system` for non-DND lanes.
