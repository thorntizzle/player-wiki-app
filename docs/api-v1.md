...
- `GET /api/v1/me` now includes the same app metadata block alongside the authenticated user payload, memberships, user preferences such as `theme_key`, `session_chat_order`, and the compatibility `frontend_mode` field, plus the admin-only `view_as` state. Flask is the stable default browser frontend again, while Gen2 is no longer account-selectable; `frontend_mode` reads still normalize to `gen2` for the compatibility API field.
