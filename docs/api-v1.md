...
- `GET /api/v1/campaigns/<campaign_slug>/characters/create` returns the system-lane create context used by Gen2. DND-5E campaigns receive a builder-ready TypeScript context for the current level-1 option slice, including enabled Systems class/species/background/subclass options, sanitized selected values, ability-score fields, preview basics, and current support-limit notes. Xianxia campaigns receive canonical Attribute/Effort/Energy/Skill fields, Martial Art/rank choices, GM grant fields, starting defaults, and import links. `POST .../characters/create` currently supports the Xianxia lane: it accepts `values`, rebuilds the authoritative backend context, validates through the Xianxia builder, writes `definition.yaml` and `import.yaml`, initializes SQLite mutable state, and returns the refreshed character plus Gen2/Flask links. DND-5E submit returns `validation_error` until the TypeScript level-one builder write path is ported. The endpoint requires the same character-management access as Flask create.
