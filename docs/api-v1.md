...
- `GET /api/v1/campaigns/<campaign_slug>/characters/create` returns the system-lane create context used by Gen2. DND-5E campaigns receive a builder-ready TypeScript context for the current level-1 option slice, including enabled Systems class/species/background/subclass options, sanitized selected values, ability-score fields, preview basics, spell-choice fields for the bounded Cleric slice, and current support-limit notes. Xianxia campaigns receive canonical Attribute/Effort/Energy/Skill fields, Martial Art/rank choices, GM grant fields, starting defaults, and import links. `POST .../characters/create` supports Xianxia native create and the narrow DND-5E PHB Fighter / PHB Barbarian / PHB Cleric with Life Domain / Human / Soldier pilot lane. The DND pilot rebuilds and validates the server-side Systems selections, writes `definition.yaml` and `import.yaml` with `builder://dnd5e-create-level-one` metadata, initializes SQLite mutable state, returns `character_exists` for duplicate slugs, and returns `validation_error` for out-of-pilot DND selections. The Cleric slice writes selected cantrips/prepared spells, Life Domain always-prepared spells, and first-level spell slots. Full DND level-one builder write parity remains pending. The endpoint requires the same character-management access as Flask create.
