...
- Combat reads return a structured tracker payload plus Gen2 live-play metadata: `live_revision`, `live_view_token`, `selected_combatant`, `selected_player_character`, `player_character_targets`, `poll_settings`, and Flask fallback links for Combat, DM Status, and DM Controls where authorized. Clients can echo `X-Live-Revision` and `X-Live-View-Token` on `GET .../combat` or `GET .../combat/live-state`; matching values return only `changed: false`, `live_revision`, and `live_view_token` so Gen2 can preserve the already-mounted player workspace. Unsupported campaign systems return `combat_system_supported: false` with an empty tracker and fallback link instead of starting live polling.
- Combat mutations are DM/admin only except for player-character vitals, which can also be updated by the assigned owner player when they provide the current sheet revision. The first Gen2 Combat slice consumes the read payload and shared Character detail/session-state contracts for the selected PC workspace; DM status, setup/seeding, source search, cleanup, and broader combat-only control lanes remain Flask-rendered until their own Gen2 parity slices land.
