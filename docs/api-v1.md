...
- The browser normal Character workflow now favors inline per-form state edits; DND-5E sheets expose the first visible inline lane for HP, resources, spell slots, inventory quantities, currency, and notes. Batch behavior is still available for compatibility and is not yet the full visible lane for every editable subpage or system.
- Browser lane boundaries are now: the normal Character page inline state slice for quick HP/resource/slot/inventory/currency/notes updates, `Session Character` for active-session edits, and `Combat`/`Encounter status` for encounter-context edits tied to `combatant=<id>`.
