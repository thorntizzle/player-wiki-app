...
- The browser normal Character workflow now favors inline per-form state edits; batch behavior is still available for compatibility and is not yet the full visible lane for every editable subpage.
- Browser lane boundaries are now: the normal Character page inline state slice for quick HP/resource/slot/inventory/notes updates, `Session Character` for active-session edits, and `Combat`/`Encounter status` for encounter-context edits tied to `combatant=<id>`.
