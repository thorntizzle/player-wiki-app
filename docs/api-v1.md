...
- Systems browsing reads are available at `GET /api/v1/campaigns/<campaign_slug>/systems`, its `/systems/search` alias, `/systems/sources`, `/systems/sources/<source_id>`, `/systems/sources/<source_id>/types/<entry_type>`, and `/systems/entries/<entry_slug>`. Together with the two app-admin import-run reads, `systems_api_routes.py` owns seven read handlers across eight GET rules plus seven mutation handlers for source policy, entry overrides, custom-entry create/update/archive/restore, and campaign item-mechanics import, for 14 handlers and 15 explicit registrations on the existing API Blueprint. Landing and search share `api.systems_index`; all other registrations retain their bare `api.*` endpoint identifiers, including `api.systems_import_run_list`, `api.systems_import_run_detail`, `api.systems_item_mechanics_import`, and the four `api.systems_custom_entry_*` identifiers. The custom-entry and item-mechanics POST mutation rules retain implicit `OPTIONS` without `HEAD`. `api.py` retains the Blueprint, 121 decorator registrations, shared decorators, request/error helpers, serializers, import-run store composition, the full DM Content Systems-payload builder, repository/service composition, dependency wiring, app-admin DND-5E ingest, and the other nonmoved Systems JSON routes.
