...
- Xianxia native creation, Xianxia manual import, Xianxia Cultivation, and DND-5E native creation now have Gen2-facing JSON contracts. Session-state JSON coverage also includes active Stance/Aura, modeled inventory add/edit/remove/equip, and Dao Immolating use request/record actions because Gen2 Session Character consumes those same durable write paths.
