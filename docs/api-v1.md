...
- Systems read endpoints follow the same source-level and entry-level visibility rules as the browser UI. Systems source updates, entry-override writes, and custom campaign entry mutations are DM/admin only. The DM Content Systems management payload is also DM/admin only and intentionally omits raw import `source_path` values.
