...
The Session ownership inventory in this section is verified on `codex/flask-rewrite-phase3b` at `44a95ba3b3f6143857c857835a9724a7b7cca16a` only. It does not yet describe `main`, the deployed app, or live production state. On that qualified branch, `player_wiki/session_api_routes.py` owns all 13 live-session JSON handlers and explicit registrations on the existing API Blueprint. `player_wiki/api.py` retains that Blueprint, 107 decorator registrations, shared request/auth/error helpers, Session serializers, service composition, and registrar dependency wiring. Character `/session/character` and character-session endpoints remain Characters-owned. The low-level `/content/...` APIs remain Publishing-owned; neither family is counted among the 13 live-session API rules.
