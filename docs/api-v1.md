...
- Systems browsing reads are available at `GET /api/v1/campaigns/<campaign_slug>/systems`, its `/systems/search` alias, `/systems/sources`, `/systems/sources/<source_id>`, `/systems/sources/<source_id>/types/<entry_type>`, and `/systems/entries/<entry_slug>`. Five handlers in `systems_api_routes.py` own those six explicit rules on the existing API Blueprint; landing and search share `api.systems_index`, and the other handlers retain their existing `api.*` identifiers. `api.py` retains the Blueprint, shared serializers, authorization/error boundaries, repository/service composition, source-policy PUT, and the remaining Systems JSON routes.
