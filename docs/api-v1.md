...
- Xianxia native creation, Cultivation/advancement, Dao Immolating use-request/recording, and manual active-state browser forms are not separate JSON API endpoints in v1; use the browser workflows for those actions, or use the low-level character content API only when you are intentionally managing the file-backed definition layer.
...
- The browser `Sheet edit view` treats that `409` as a safe reload boundary: it refreshes the latest sheet revision, restores the pending page-local draft, and asks the user to review before saving again instead of dropping the whole batch.
- The Character-page `Sheet edit view` is intentionally a first bounded subset rather than an umbrella for every editable subpage. First-pass batch saves cover Quick Reference vitals/resources/slot usage, Inventory quantities/currency, and Personal/Notes text, while spell-list changes, equipment-state or portrait updates, broader inventory maintenance, rests, and advanced editor flows stay on their existing routes.
- Browser UX keeps the edit lanes distinct: Character-page `Sheet edit view` is the out-of-session batch lane, `Session Character` keeps immediate active-session edits, and `Combat`/`Encounter status` keeps encounter-context edits tied to `combatant=<id>`.
