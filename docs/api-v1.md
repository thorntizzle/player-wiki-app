...
- Systems browsing reads are available at `GET /api/v1/campaigns/<campaign_slug>/systems`, its `/systems/search` alias, `/systems/sources`, `/systems/sources/<source_id>`, `/systems/sources/<source_id>/types/<entry_type>`, and `/systems/entries/<entry_slug>`. `systems_api_routes.py` owns five read handlers across those six rules plus six mutation handlers for source policy, entry overrides, and custom-entry create, update, archive, and restore, for 11 handlers and 12 explicit registrations on the existing API Blueprint. Landing and search share `api.systems_index`; all other registrations retain their bare `api.*` endpoint identifiers, including the four `api.systems_custom_entry_*` identifiers. The custom-entry mutation rules retain implicit `OPTIONS` without `HEAD`. `api.py` retains the Blueprint, shared decorators, request/error helpers, serializers, the full DM Content Systems-payload builder, repository/service composition, dependency wiring, and the remaining Systems JSON routes such as campaign item mechanics import and admin ingest.
