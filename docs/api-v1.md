...
- The DND-5E API ingest preserves its current audit and partial-failure behavior: it writes no `AuthStore` audit event, and completed import runs and shared-entry replacements can remain durable if import-run refetch or response serialization later fails. The separately audited browser import contract is unchanged. An approved security correction to add the API audit is pending and is not part of the shipped contract described here.
